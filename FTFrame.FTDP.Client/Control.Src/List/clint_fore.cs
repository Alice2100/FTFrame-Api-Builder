...
namespace ft_list
{
    ...
    public class List : Control, INamingContainer
    {
        ...
        public static string PartXML3 = "lstr}表示sql取值(支持@语法),@getConst(index)表示获取图文信息，@str表示文字常量，最后一个参数表示该列文字的链接，为空表示无链接，$0|0|按钮文字|地址|_self,$表示链接文字或按钮，第一个数字表示仅仅为该状态值时才显示该链接文字或按钮，为空表示所有状态值，第二个数字为0表示为链接文字，否则表示为按钮，第四个字符表示打开的新窗口的地址，支持@和[]语法，第五个参数为打开方式。传递的关键参数为dsformid，dsformpid。一列里面需要显示多项可以用分号隔开。$0,1|0|多项匹配时可以用逗号隔开。~0|(0|a1)(1|a2)(other|a3)表示0级流程不同状态值时显示的图文。!abcd(null|x1)(|x2)(val0|x3)(other|x4)表示列abcd的值分别为不存在、为空、val0或其他时显示的图文。若列定义后加&&&即为定义该列的开关条件，支持@语法，例如@session[abc]==\"1\"、@session[abc]!=\"1\"、@session[abc]=?\",1,\"、@session[abc]!?\",1,\"（=?表示包含,!?表示不包含）。多列用|||隔开。用[rowid]来获取指定列的当前值。]]></description><class>string</class><default><![CDATA[项目名称#UN1btIeLgtvMSO4102QtPg==#30%;center#proname#alert('[fid]'+'[pid]')#_self#&amp;&amp;&amp;|||添加人#vNtbjABXfgQSymG2mTM6Rw==#20%;center####&amp;&amp;&amp;|||添加时间#7Fu4ag/rp4T5GGfQjcHxIw==#30%;center#addtime###&amp;&amp;&amp;|||#FUd2Fiu2dXpByB8TPQYnLLnXikHNiNmTZcyQx1d/few=#20%;center####&amp;&amp;&amp;]]></default><category>1.基础</category></param><param><name>IsAutoShow</name><type>int</type><caption>2.是否自动显示</caption><description>定义是否自动显示，手动显示需要执行load_[partid](1)方法</description><class>enum</class><default>1</default><category>1.基础</category><enums><enum>手动</enum><enum>自动</enum></enums></param><param><name>CusCondition</name><type>string</type><caption>3.自定义条件</caption><description>自定义查询SQL条件，支持@code接口，{para}写法，例如and a.flow!=8。支持@语法。为空表示不设置。设置全自定义SQL时失效。</description><class>string</class><default /><category>2.高级</category></param><param><name>RateEle</name><type>string</type><caption>4.动态新增行列名</caption><description>默认以fid为主标识显示，此处定义为以fid和动态新增行列名为主标识显示。输入值为动态新增行的列名。可用于动态新增行的列表页显示</description><class>string</class><default /><category>2.高级</category></param><param><name>CusSQL</name><type>string</type><caption>1.全自定义查询</caption><description>";
        public static string PartXML4 = "全自定义列表查询SQL，支持@code接口，{para}、@p1@写法，@FromList:@code语法表示数据源来自于代码方法</description><class>string</class><default /><category>2.高级</category></param><param><name>CusSQLHalf</name><type>string</type><caption>2.记录数查询语句</caption><description>为空时根据全自定义查询获取记录总数，支持@code和sql@code接口，{para}写法。</description><class>string</class><default /><category>2.高级</category></param><param><name>RerationTree</name><type>string</type><caption>5.级联显示</caption><description>设置为级联显示并指定参数，为空表示不设置。三个参数用分号隔开，例如，fid;pid;pid='@request[dsformid]'，分别表示主键字段、父级字段、第一级的筛选条件（支持@语法，{para}写法）。</description><class>string</class><default /><category>2.高级</category></param><param><name>BlockDataDefine</name><type>string</type><caption>7.数据渲染模板</caption><description><![CDATA[定义数据渲染模板，格式为HTML代码，支持@和[]语法，例如<div style=\"float:left;width:100px;height:100px\">{check}[proname]</div>，为空为不设置，{check}表示复选或单选框，支持code接口]]></description><class>string</class><default /><category>2.高级</category></param><param><name>AppendTitle</name><type>string</type><caption>8.附加表头</caption><description><![CDATA[定义附加表头，格式为HTML代码，支持@语法，例如<tr><th colspan=2>2015</th><th colspan=2>2016</th></tr>，为空为不设置，支持code接口]]></description><class>string</class><default /><category>2.高级</category></param><param><name>LoadEndJS</name><type>string</type><caption>9.载入后JS</caption><description><![CDATA[定义载入后JS，即load方法异步数据获取完毕后执行的JS，不能带双引号]]></description><class>string</class><default /><category>2.高级</category></param><param><name>ExecBefore</name><type>string</type><caption>11.获取前执行</caption><description></description><class>string</class><default /><category>设置</category></param><param><name>ExecAfter</name><type>string</type><caption>12.获取后执行</caption><description></description><class>string</class><default /><category>设置</category></param><param><name>CustomConnection</name><type>string</type><caption>A.自定义数据库连接</caption><description><![CDATA[指定该数据列表从非当前数据库中获";
        ...
        public string getHtml(string theControlID, string SetStyle, int SelectType, string LoadingImg, int RateNumType, string SelectOnClick, int NumsPerPage, string FirstPage, string PrePage, string NextPage, string LastPage, string TurnPage, string CountZero, int Cellspacing, int Cellpadding, string ColorHover, string ColorDefault, string ColorSelect, int HeadIsShow, int TurnIsShow, string OrderBy, string NewSiteID, string FormStartTime, string FormEndTime, string FormPID, string MainTable, string RowAll, int IsAutoShow, string CusCondition, string RateEle, string CusSQL, string CusSQLHalf, string RerationTree, string BlockDataDefine, string AppendTitle, string LoadEndJS, string CustomConnection, string CustomTurnPageBottom, string CustomTurnPageTop, string ExecBefore, string ExecAfter, string CacuRowData, int IsSearchShow, int IsCusCdnShow, string SchAreaApdHtml, string AdvSearch, string AdvSearchCaption, string SearchDefine, string SearchDefineTip, string StrictSearch, int IsRefreshShow, int IsColDefineShow, string CusCdnCols, string Const1, string Const2, string Const3, string Const4, string Const5, string Const6, string Const7, string Const8, string Const9, string Const10, string Const11, string Const12, string Const13, string Const14, string Const15, string Const16, string Const17, string Const18, string Const19, string Const20, string Const21, string Const22, string Const23, string Const24, string List_OPID, string List_Code, string MemBind, string EleCondition, string RoleBindData, string RoleBindSession, string AuthRule, string FlowStat, string Del_Code, string Del_MemBind, string Del_EleCondition, string Del_RoleBindData, string Del_RoleBindSession, string Del_AuthRule, string Del_FlowStat, string Del_OPID, string Flow_Code, string Flow_MemBind, string Flow_EleCondition, string Flow_RoleBindData, string Flow_RoleBindSession, string Flow_AuthRule, string Flow_FlowStat, string Flow_OPID, string Copy_Code, string Copy_MemBind, string Copy_EleCondition, string Copy_RoleBindData, string Copy_RoleBindSession, string Copy_AuthRule, string Copy_FlowStat, string Copy_OPID, int ExportMax, int MenuIsShow, string MenuButtonSet, string APISet)
        {
            ...
            if (MenuIsShow == 1)
            {
                ...
                foreach (string MenuItem in MenuItems)
                {
                    ...
                    if (colname != "")
                    {
                        btns += ftdpcontroltemp.TextLinkDemoStyle2("btn" + loop, "", colname);
                    }
                }
                ...
                if (IsSearchShow == 1)
                {
                    ...
                    schs += ftdpcontroltemp.TextLinkDemoStyle2("", "", ftdpcontroltemp.TextShow.ChaXun);
                    if (IsCusCdnShow == 1)
                    {
                        schs += ftdpcontroltemp.TextLinkDemoStyle2("", "", ftdpcontroltemp.TextShow.ZiDingYi);
                    }
                    if (!AdvSearch.Equals(""))
                    {
                        schs += ftdpcontroltemp.TextLinkDemoStyle2("", "", AdvSearchCaption == "" ? ftdpcontroltemp.TextShow.GaoJi : AdvSearchCaption);
                    }
                }
                if (IsColDefineShow == 1)
                {
                    schs += ftdpcontroltemp.TextLinkDemoStyle2("", "", ftdpcontroltemp.TextShow.LieDingYi);
                }
                if (IsRefreshShow == 1)
                {
                    schs += ftdpcontroltemp.TextLinkDemoStyle2("", "", ftdpcontroltemp.TextShow.ShuaXin);
                }

                s = ftdpcontroltemp.Tableframe(btns, SchAreaApdHtml + "<span style='padding:0px;margin:0px;float:right'>" + schs + "</span>");
            }
            ...
            for (int rowi = 0; rowi < rows.Length; rowi++)
            {
                ...
                var descs = rowcols[5].Split(new string[] { " ", ",", ";", "(", ":", "；", "：", "（", "，" }, StringSplitOptions.RemoveEmptyEntries);
                ColumnTitle.Add(rowcols[0] + (descs.Length == 0 ? "" : ("<br><font color='#999999' style='white-space: nowrap'>" + descs[0]+"</font>")));
                ...
                for (int i = 0; i < coldatas.Length; i++)
                {
                    ...
                    else if (coldatas[i].IndexOf("@getPValue(") == 0) coldatas[i] = coldatas[i].Replace("@getPValue(", "");
                    else if (coldatas[i].IndexOf("@getValueAll(") == 0) coldatas[i] = coldatas[i].Replace("@getValueAll(", "");
                    else if (coldatas[i].IndexOf("@getPValueAll(") == 0) coldatas[i] = coldatas[i].Replace("@getPValueAll(", "");
                    else if (coldatas[i].IndexOf("@SQL{") == 0) coldatas[i] = "(SQL)";
                    ...
                    else if (coldatas[i].IndexOf("@getConst") == 0) coldatas[i] = coldatas[i].Replace(coldatas[i], Consts[int.Parse(coldatas[i].Substring(coldatas[i].IndexOf('(') + 1, coldatas[i].IndexOf(')') - coldatas[i].IndexOf('(') - 1)) - 1]);
                    else if (coldatas[i].IndexOf("@code(") == 0)
                    {
                        coldatas[i] = coldatas[i].Substring(coldatas[i].IndexOf('(') + 1);
                        if (coldatas[i].IndexOf(',') < 0) coldatas[i] = coldatas[i].Replace(")", "");
                        else coldatas[i] = coldatas[i].Substring(0, coldatas[i].IndexOf(','));
                    }
                    else if (coldatas[i].IndexOf("@para{") == 0)
                    {
                        coldatas[i] = coldatas[i].Substring(coldatas[i].IndexOf('{') + 1);
                        if (coldatas[i].IndexOf(',') < 0) coldatas[i] = coldatas[i].Replace("}", "");
                        else coldatas[i] = coldatas[i].Substring(0, coldatas[i].IndexOf(','));
                    ...
                    else if (coldatas[i].IndexOf("~") == 0 || coldatas[i].IndexOf("!") == 0)
                    {
                    ...
                    else if (coldatas[i].IndexOf("@KeyValue", StringComparison.CurrentCultureIgnoreCase) == 0)
                    {
                        coldatas[i] = "(KeyValue)";
                    }
                    else if (coldatas[i].StartsWith("@"))
                    {
                    ...
                    if (isAdv) coldatas[i] = clr(coldatas[i]);
                    coldata += coldatas[i];
            ...
            if (HeadIsShow == 1)
            {
                if (AppendTitle.Trim().StartsWith("<"))
                {
                    s += AppendTitle;
                }
