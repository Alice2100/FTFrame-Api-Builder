...
try
{
    ...
    if (MenuIsShow == 1)
    {
        ...
        {
         if(colname[i].Equals(""))continue;
            string clickstr = "";
            if (colcheck[i].Equals("common"))
            {
                switch (colrole[i])
                {
                    case "del":
				...
				if(colrole[i].StartsWith("flow"))
				{
          if(colrole[i].IndexOf(":")>0)
          {
            clickstr = "onclick=\"" + "if(confirm('"+colrole[i].Split(':')[1]+"'))dl_flow(this,'" + SiteID + "','" + PartID + "','"+colrole[i].Split(':')[0].Substring(4)+"')\"";
          }
          else
					clickstr = "onclick=\"" + "dl_flow(this,'" + SiteID + "','" + PartID + "','"+colrole[i].Substring(4)+"')\"";
				}
				if(colrole[i].StartsWith("saveto("))
				{
					clickstr = "onclick=\"" + "dl_saveto(this,'"+colrole[i].Replace("saveto(","").Replace(")","")+"','" + SiteID + "','" + PartID + "')\"";
				}
            }
            else if (colcheck[i].Equals("one"))
            {
                clickstr = "onclick=\"if(dl_SelectOne(this)!=null){" + colendurer[i].Replace("[id]","dl_SelectOne(this)") + "}\"";
            }
            else if (colcheck[i].Equals("more"))
            {
                clickstr = "onclick=\"if(dl_SelectMore(this,true)!=null){" + colendurer[i].Replace("[ids]","dl_SelectMore(this,true)") + "}\"";
            }
            else
            {
              clickstr = "onclick=\"" + colendurer[i] + "\"";
            }
            /*
            else if (colcheck[i].Equals("none"))
            {
                clickstr = "onclick=\"" + "dl_openWindow(" + colendurer[i] + ")\"";
            }
            */
            btns += ftdpcontroltemp.TextLinkDemoStyle2("btn" + i + "_" + PartID, clickstr, colname[i]);
            ButtonStyleJs+=ftdpcontroltemp.ButtonStyleInitJs("btn"+i+"_"+PartID,MenuOppics[i]);
        }
        string schs = "";
        if (IsSearchShow == 1)
        {
            if(!SearchDefine.Equals(""))
            {
                ...
                {
                  schs += ftdpcontroltemp.SearchText(PartID, PartID, PartID, SearchDefine);
                }
                else
                {
                  schs += ftdpcontroltemp.SearchTextTip(PartID, SearchDefineTip,PartID,PartID, SearchDefine,PartID);
                }
            }
            ...
            foreach (string strictsearch1 in strictsearch)
            {
                ...
                schs += ftdpcontroltemp.SearchSelectStart(strictsearch1.Split('|')[0].Split(';')[0], PartID) + options + ftdpcontroltemp.SearchSelectEnd();
            }
            //list_adv_s_"+PartID+"='';
            schs += ftdpcontroltemp.TextLinkDemoStyle2("btns_" + PartID,"onclick=\"dl_search('"+PartID+"')\"", "查询");
            ButtonStyleJs += ftdpcontroltemp.ButtonStyleInitJs("btns_" + PartID, "ui-icon-search");
            if (IsCusCdnShow==1)
            {
                schs += ftdpcontroltemp.TextLinkDemoStyle2("btnc_" + PartID, "onclick=\"dl_cuscdn('"+PartID+"','"+CusCdnCols+"')\"", "自定义");
                ButtonStyleJs += ftdpcontroltemp.ButtonStyleInitJs("btnc_" + PartID, "ui-icon-search");
            }
            if (!AdvSearch.Equals(""))
            {
                schs += ftdpcontroltemp.TextLinkDemoStyle2("btna_" + PartID, "onclick=\"" + AdvSearch + "\"", AdvSearchCaption==""?"高级":AdvSearchCaption);
                ButtonStyleJs += ftdpcontroltemp.ButtonStyleInitJs("btna_" + PartID, "ui-icon-search");
            }
        }
        if(IsColDefineShow==1)
        {
            schs += ftdpcontroltemp.TextLinkDemoStyle2("btncoldefine_" + PartID, "onclick=\"dl_coldefine('" + PartID + "',l_cd_" + PartID + ")\"", "列定义");
            ButtonStyleJs += ftdpcontroltemp.ButtonStyleInitJs("btncoldefine_" + PartID, "ui-icon-gear");
        }
        if(IsRefreshShow==1)
        {
            schs += ftdpcontroltemp.TextLinkDemoStyle2("btnf_" + PartID, "onclick=\"dl_refresh('" + PartID + "')\"", "刷新");
  ...
  string ColDefine_Default = "";
    ...
    */
    ...
    string JSTableTail=ftdpcontroltemp.ListTail();
      ...
      s+="<script language=\"javascript\" src=\"/lib/list.res/func1712271.js\"></script>";
