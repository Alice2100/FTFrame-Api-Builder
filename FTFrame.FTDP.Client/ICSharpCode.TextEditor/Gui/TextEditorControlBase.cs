...
namespace ICSharpCode.TextEditor
{
	...
	public abstract class TextEditorControlBase : UserControl
	{
		...
		public bool Find(string text)
		{
			...
			var offset = this.Text.IndexOf(text, startIndex, StringComparison.CurrentCultureIgnoreCase);
			if (offset >= 0)
			{
				//findIndex = offset + text.Length;//为查找下一个做准备
				var start = this.Document.OffsetToPosition(offset);
				var end = this.Document.OffsetToPosition(offset + text.Length);
				this.ActiveTextAreaControl.SelectionManager.SetSelection(new DefaultSelection(this.Document, start, end));

				//滚动到选择的位置。
				this.ActiveTextAreaControl.Caret.Position = end;
				this.ActiveTextAreaControl.TextArea.ScrollToCaret();
				return true;
			}
			else
			{
