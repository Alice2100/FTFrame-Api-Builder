...
namespace ICSharpCode.TextEditor.Gui.CompletionWindow
{
    ...
    public class DeclarationViewWindow : Form, IDeclarationViewWindow
    {
        ...
        Font NewFont = new Font(Control.DefaultFont.FontFamily, 15);
        public int GetRequiredLeftHandSideWidth(Point p)
        {
            if (description != null && description.Length > 0)
            {
                ...
                {
                    Size s = TipPainterTools.GetLeftHandSideDrawingSizeHelpTipFromCombinedDescription(this, g, NewFont, null, description, p);
                    return s.Width;
                }
            }
            return 0;
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            if (description != null && description.Length > 0)
            {
                if (fixedWidth)
                {
                    TipPainterTools.DrawFixedWidthHelpTipFromCombinedDescription(this, pe.Graphics, NewFont, null, description);
                }
                else
                {
                    TipPainterTools.DrawHelpTipFromCombinedDescription(this, pe.Graphics, NewFont, null, description);
                }
            }
        }
