			...
			while (curItem < completionData.Length && yPos < Height) {
				...
				if (drawingBackground.IntersectsWith(pe.ClipRectangle)) {
					...
					if (imageList != null && completionData[curItem].ImageIndex < imageList.Images.Count) {
						...
						g.DrawImage(imageList.Images[completionData[curItem].ImageIndex], new RectangleF(1 + (imageWidth - imageList.ImageSize.Width) / 2, yPos+(itemHeight-imageList.ImageSize.Height)/2, imageList.ImageSize.Width, imageList.ImageSize.Height));
						xPos = imageWidth;
					}
