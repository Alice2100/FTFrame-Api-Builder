...
namespace FTDPClient.Page
{
        ...
        */
        ...
        public static string CloneControl(string ControlCaption, string ControlID)
        {
            ...
            try
            {
                ...
                dr.Close();
                foreach(string s in sqls)db3.execSql(s);
                return NewControlID;
            }
            catch (Exception ex)
            {
        ...
        public static bool AddPart2Editor(string partid, Editor edr)
        {
            ...
            try
            {
                if (edr != null && partid != null)
                {
                    ...
                    rdr.Close();
                    //
                    if (edr.editocx.pasteHtml("<br><br>" + globalConst.PageWare.getControlEditHead(partid, PartName, "1px", "1px", WareName) + getPartHtml(WareID, WareName, PartName, PartXml, AsPortal) + globalConst.PageWare.getControlEditTail() + "<br><br>"))
                    {
            ...
            try
            {
                ...
                if (IsNowAdd2Page)
                {
                    ...
                    if (ed != null)
                    {
                        ...
                        if (ed.editocx.pasteHtml("<br><br>" + globalConst.PageWare.getControlEditHead(FirstPartID, FirstPartName, "1px", "1px", WareName) + getPartHtml(WareID, WareName, FirstPartName, FirstPartXml, AsPortal) + globalConst.PageWare.getControlEditTail() + "<br><br>"))
                        {
        ...
        public static string SingleTableQuickControl(string MainTable, string TableDesc, string PageId)
        {
            try
            {
                #region PartParaValue
                string connstr = Options.GetSystemDBSetConnStr();
                var dbtype = Options.GetSystemDBSetType();
                var defaultColumns = Adv.GetDefaultColumns(MainTable).Select(r => r.ToLower());
                var Columns = new List<(string Name, string Desc, bool IsDefault)>();
                if (dbtype==globalConst.DBType.MySql)
                {
                    using (MySqlConnection conn = new MySqlConnection(connstr))
                    {
                        ...
                        using (MySqlDataReader dr = new MySqlCommand(sql, conn).ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                var colName = dr.GetString(dr.GetOrdinal("Field"));
                                var colDesc = dr.IsDBNull(dr.GetOrdinal("Comment")) ? colName : dr.GetString(dr.GetOrdinal("Comment"));
                                var isDefault = defaultColumns.Contains(colName.ToLower());
                                Columns.Add((colName, colDesc, isDefault));
                            }
                        }
                    }
                }
 ...
 from sys.columns, sys.tables, sys.types where sys.columns.object_id = sys.tables.object_id and sys.columns.user_type_id=sys.types.user_type_id and sys.tables.name='" + MainTable + "')";
                        using (SqlDataReader dr = new SqlCommand(sql, conn).ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                var colName = dr.GetString(0);
                                var colDesc = dr.IsDBNull(dr.GetOrdinal("description")) ? colName : dr.GetString(dr.GetOrdinal("description"));
                                var isDefault = defaultColumns.Contains(colName.ToLower());
                                Columns.Add((colName, colDesc, isDefault));
                            }
                        }
                ...
                string orderCol = "id";
                if (defaultColumns.Count() > 0) orderCol = defaultColumns.First();
                else orderCol = Columns.First().Name;

                // CusSQL select * FROM ft_ftdp_apidoc WHERE 1=1
                // MainTable @ft_ftdp_apidoc
                // OrderBy order by orderCol desc
                // RowAll id#U7WOCyyikHc41Ji1f+sCzQ==#auto;left###资讯id##0&&&|||title#R7ZVLnenz3UUhtF8Qc5dVA==#auto;left###题名##0&&&|||news_type#nWlo0O3ROkVs++DgyGLrnw==#auto;left###资讯类别##0&&&
                // APISet NewsList[##]获取资讯新闻列表[##]orderBy,orderType,schText,schStrict,pageSize,numType,pageNum[##][#][#][#][#]12[#][#][##]排序字段[#]asc或desc[#]多字段或按字段模糊查询[#]完全匹配查询，例如a:1;b:2[#]每页显示数量[#]默认无序号 1按每页排序号，2按全部排序号[#]显示第几页，默认第一页[##]json[##]ExportFileName[::][;;]

                var List_CusSQL = $"SELECT * FROM {MainTable} WHERE 1=1";
                var List_MainTable = $"@{MainTable}";
                var List_OrderBy = $"order by {orderCol} desc";
                StringBuilder List_RowAll = new StringBuilder();
                for(var i=0;i< Columns.Count;i++)
                {
                    if (i > 0) List_RowAll.Append("|||");
                    List_RowAll.Append($"{Columns[i].Name}#{str.getEncode(Columns[i].Name)}#auto;left###{Columns[i].Desc}##0&&&");
                }
                var List_APISet = $"list[##]{TableDesc}_{res.ctl.str("List_Api.1")}[##]orderBy,orderType,schText,schStrict,pageSize,numType,pageNum[##][#][#][#][#]12[#][#][##]{res.ctl.str("List_Api.2")}[##]json[##]ExportFileName[::][;;]";

                // Define 客户姓名##add_name##yDk62ilg8/wcXZiSusi+KycqCF8U3EVFwbI+9l6dpxI=##ckbaz6NNvOcIOCf1mwZpow==##noempty####add_name##m1U8o6GpviGj96AwAELxMw==|||##add_lianxiren##vOocEAcYK8wnnwId4T2EMgj2GjJ8V49kCGWQS3Mq1y0=##ckbaz6NNvOcIOCf1mwZpow==######add_lianxiren##m1U8o6GpviGj96AwAELxMw==|||##add_dianhua##vOocEAcYK8wnnwId4T2EMuOqj/DDJc1WuO+ADbJOFiA=##ckbaz6NNvOcIOCf1mwZpow==######add_dianhua##m1U8o6GpviGj96AwAELxMw==|||##add_date##H3+pCUzW3ykaXUwvdfg+v/yWuP4mlwOBrUP3XlfAkxw=##ckbaz6NNvOcIOCf1mwZpow==######add_date##m1U8o6GpviGj96AwAELxMw==
                // APISet add[##]新增[##]name[#]phone[#]sex[#]education[#]birthday[#]mimo[#]uploadfile[#]uploadimage[#]multifiles[#]multiimages[#]introduce_ishtml[#]area_province[#]area_city[#]area_region[##]name[#]phone[#]sex[#]education[#]birthday[#]mimo[#]uploadfile[#]uploadimage[#]multifiles[#]multiimages[#]introduce_ishtml[#]area_province[#]area_city[#]area_region[##]客户姓名[#]客户电话[#]性别[#]学历(代码)[#]生日[#]描述[#]附件[#]图片[#]多个文件[#]多个图片[#]介绍[#]省份[#]市[#]区县[##]json

                StringBuilder DataOP_Add_Define = new StringBuilder();
                StringBuilder DataOP_Add_APISet_1 = new StringBuilder();
                StringBuilder DataOP_Add_APISet_2 = new StringBuilder();
                StringBuilder DataOP_Add_APISet_3 = new StringBuilder();
                var loopI = 0;
                for (var i = 0; i < Columns.Count; i++)
                {
                    if (Columns[i].IsDefault) continue;
                    if (loopI > 0)
                    {
                        DataOP_Add_Define.Append("|||");
                        DataOP_Add_APISet_1.Append("[#]");
                        DataOP_Add_APISet_2.Append("[#]");
                        DataOP_Add_APISet_3.Append("[#]");
                    }
                    DataOP_Add_Define.Append($"{Columns[i].Desc}##{Columns[i].Name}##{str.getEncode("@"+MainTable+"."+ Columns[i].Name)}##{str.getEncode("0")}######add_{Columns[i].Name}##{str.getEncode("")}");
                    DataOP_Add_APISet_1.Append(Columns[i].Name);
                    DataOP_Add_APISet_2.Append(Columns[i].Name);
                    DataOP_Add_APISet_3.Append(Columns[i].Desc);
                    loopI++;
                }
                var DataOP_Add_APISet = $"add[##]{TableDesc}_{res.ctl.str("DataOP_Api.1")}[##]{DataOP_Add_APISet_1.ToString()}[##]{DataOP_Add_APISet_2.ToString()}[##]{DataOP_Add_APISet_3.ToString()}[##]json";

                StringBuilder DataOP_Mod_Define = new StringBuilder();
                StringBuilder DataOP_Mod_APISet_1 = new StringBuilder();
                StringBuilder DataOP_Mod_APISet_2 = new StringBuilder();
                StringBuilder DataOP_Mod_APISet_3 = new StringBuilder();
                loopI = 0;
                for (var i = 0; i < Columns.Count; i++)
                {
                    if (Columns[i].IsDefault) continue;
                    if (loopI > 0)
                    {
                        DataOP_Mod_Define.Append("|||");
                        DataOP_Mod_APISet_1.Append("[#]");
                        DataOP_Mod_APISet_2.Append("[#]");
                        DataOP_Mod_APISet_3.Append("[#]");
                    }
                    DataOP_Mod_Define.Append($"{Columns[i].Desc}##{Columns[i].Name}##{str.getEncode("@" + MainTable + "." + Columns[i].Name)}##{str.getEncode("1")}######mod_{Columns[i].Name}##{str.getEncode("")}");
                    DataOP_Mod_APISet_1.Append(Columns[i].Name);
                    DataOP_Mod_APISet_2.Append(Columns[i].Name);
                    DataOP_Mod_APISet_3.Append(Columns[i].Desc);
                    loopI++;
                }
                var DataOP_Mod_APISet = $"mod[##]{TableDesc}_{res.ctl.str("dataop.api.1")}[##]{DataOP_Mod_APISet_1.ToString()}[##]{DataOP_Mod_APISet_2.ToString()}[##]{DataOP_Mod_APISet_3.ToString()}[##]json";

                // Define 客户姓名##name##o5eXpasUkSlKD3GBmFr/AmbbYHqwyKP1bwloUO6aBFY=####1##0##0##m1U8o6GpviGj96AwAELxMw==|||客户电话##phone##OPg2bUcOtOSTrIQ+rA9qKbaqqB6JH2LzIdgx34cxk1A=####1##0##0##m1U8o6GpviGj96AwAELxMw==
                // APISet get[##]详情,参数1 fid[##]name[#]phone[#]sex[#]education[#]birthday[#]mimo[#]uploadfile[#]uploadimage[#]multifiles[#]multiimages[#]introduce_ishtml[#]area_province[#]area_city[#]area_region[##]name[#]phone[#]sex[#]education[#]birthday[#]mimo[#]uploadfile[#]uploadimage[#]multifiles[#]multiimages[#]introduce_ishtml[#]area_province[#]area_city[#]area_region[##]客户姓名[#]客户电话[#]性别[#]学历(代码)[#]生日[#]描述[#]附件[#]图片[#]多个文件[#][#]介绍[#]省份[#]市[#]区县[##][#][#][#][#][#][#][#][#][#][#][#][#][#]

                StringBuilder DyValue_Define = new StringBuilder();
                StringBuilder DyValue_APISet_1 = new StringBuilder();
                StringBuilder DyValue_APISet_2 = new StringBuilder();
                StringBuilder DyValue_APISet_3 = new StringBuilder();
                StringBuilder DyValue_APISet_4 = new StringBuilder();
                loopI = 0;
                for (var i = 0; i < Columns.Count; i++)
                {
                    if (loopI > 0)
                    {
                        DyValue_Define.Append("|||");
                        DyValue_APISet_1.Append("[#]");
                        DyValue_APISet_2.Append("[#]");
                        DyValue_APISet_3.Append("[#]");
                        DyValue_APISet_4.Append("[#]");
                    }
                    DyValue_Define.Append($"{Columns[i].Desc}##{Columns[i].Name}##{str.getEncode("@" + MainTable + "." + Columns[i].Name)}####1##0##0##{str.getEncode("")}");
                    DyValue_APISet_1.Append(Columns[i].Name);
                    DyValue_APISet_2.Append(Columns[i].Name);
                    DyValue_APISet_3.Append(Columns[i].Desc);
                    DyValue_APISet_4.Append("");
                    loopI++;
                }
                var DyValue_APISet = $"get[##]{TableDesc}_{res.ctl.str("dyvalue.api.1")}[##]{DyValue_APISet_1.ToString()}[##]{DyValue_APISet_2.ToString()}[##]{DyValue_APISet_3.ToString()}[##]{DyValue_APISet_4.ToString()}";
                #endregion

                var reStr = "";

                Dictionary<string, string> dic = new Dictionary<string, string>();
                dic.Add("CusSQL", List_CusSQL);
                dic.Add("MainTable", List_MainTable);
                dic.Add("OrderBy", List_OrderBy);
                dic.Add("RowAll", List_RowAll.ToString());
                dic.Add("APISet", List_APISet);

                reStr += "<br><br>"+ ControlAdd("list",TableDesc+"_List",PageId,dic) + "<br><br>";

                dic = new Dictionary<string, string>();
                dic.Add("Define", DataOP_Add_Define.ToString());
                dic.Add("APISet", DataOP_Add_APISet);

                reStr += "<br><br>" + ControlAdd("dataop", TableDesc + "_Add", PageId, dic) + "<br><br>";

                dic = new Dictionary<string, string>();
                dic.Add("Define", DataOP_Mod_Define.ToString());
                dic.Add("APISet", DataOP_Mod_APISet);

                reStr += "<br><br>" + ControlAdd("dataop", TableDesc + "_Mod", PageId, dic) + "<br><br>";

                dic = new Dictionary<string, string>();
                dic.Add("Define", DyValue_Define.ToString());
                dic.Add("APISet", DyValue_APISet);

                reStr += "<br><br>" + ControlAdd("dyvalue", TableDesc + "_Detail", PageId, dic) + "<br><br>";

                return reStr;

                string ControlAdd(string WareName,string WareCaption,string pageId, Dictionary<string, string> paraDic)
                {
                    DB db3 = new DB();
                    db3.Open(db.ConnStr_Site());
                    try
                    {
                        ...
                        string Control_Paras = globalConst.ConfigConn.GetString(sql);
                        if (Control_Paras.Trim().Equals("")) Control_Paras = "<paras></paras>";
                        sql = "insert into controls(id,name,caption,datasource,paras,addtime)values('" + WareID + "','" + WareName + "','" + str.D2DD(WareCaption) + "','" + DataSourceID + "','" + Control_Paras.Replace("'", "''") + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", System.Globalization.DateTimeFormatInfo.InvariantInfo) + "')";
                        globalConst.CurSite.SiteConn.execSql(sql);
                        ...
                        while (rdr.Read())
                        {
                            ...
                            for (i = 0; i < NdList.Count; i++)
                            {
                                ...
                                if (!Nd.SelectSingleNode("class").InnerText.StartsWith("system[@"))
                                {
                                    ...
                                    partXml += Nd.SelectSingleNode("type").OuterXml;
                                    partXml += "<value>";
                                    var paraName = Nd.SelectSingleNode("name").InnerText;
                                    if (paraDic.ContainsKey(paraName))
                                    {
                                        partXml += paraDic[paraName].Replace("&", "&amp;").Replace("<", "&lt;").Replace(">", "&gt;");
                                    }
                                    else partXml += Nd.SelectSingleNode("default").InnerText;
                                    partXml += "</value>";
                                    partXml += "</param>";
                                }
                            }
                        ...
                        if (pageId!=null)
                        {
                            sql = "insert into part_in_page(pageid,partid)values('" + PageId + "','" + FirstPartID + "')";
                            globalConst.CurSite.SiteConn.execSql(sql);
                        }

                        return $"<ftdp:control id={FirstPartID} name=\"{FirstPartName}\" height=\"1px\" width=\"100%\"></ftdp:control>";
                    }
                    finally
                    {
                        db3.Close();
        ...
        public static void refreshApisInSitePage(string controlname, string partid, TreeNode PartNode)
        {
            try
            {
                ...
                if (apiset != null)
                {
                    string[] items1 = apiset.Split(new string[] { "{$$}" }, StringSplitOptions.RemoveEmptyEntries);
                    foreach (string item1 in items1)
                    {
                        string[] items2 = item1.Split(new string[] { "[##]" }, StringSplitOptions.None);
                        TreeNode apiNode = new TreeNode(items2[0] + "_" + items2[1], 32, 32);
                        string[] tagapip = new string[6];
                        tagapip[0] = items2[0] + "_" + items2[1];
                        tagapip[1] = items2[0] + "_" + items2[1];
                        tagapip[2] = rdm.getID() + "_fapi";
                        tagapip[3] = controlname;
                        tagapip[4] = partid;
                        tagapip[5] = items2[0];
                        apiNode.Tag = tagapip;
                        PartNode.Nodes.Add(apiNode);
                    }
                }
                PartNode.Expand();
            }
        ...
        public static void refreshPartsInSiteTree(string pageid)
        {
            ...
            try
            {
                ...
                while (pdr.Read())
                {
                    ...
                    cpdr.Close();
                    int imageIndex = 20;
                    if (controlname == "dataop") imageIndex = 31;
                    else if (controlname == "dyvalue") imageIndex = 30;
                    else if (controlname == "list") imageIndex = 29;
                    if (globalConst.siteTreeShowColName.Equals("name"))
                    {
                        ndp = new TreeNode(thisctrlcaption + "_" + controlname + "_" + partname, imageIndex, imageIndex);
                    }
                    else
                    {
                        ndp = new TreeNode(thisctrlcaption + "_" + controlcaption + "_" + partcaption, imageIndex, imageIndex);
                    }
                    string partid = pdr.getString("partid");
                    string[] tagp = new string[4];
                    tagp[0] = thisctrlcaption + "_" + controlname + "_" + partname;
                    tagp[1] = thisctrlcaption + "_" + controlcaption + "_" + partcaption;
                    tagp[2] = pdr.getString("partid");
                    tagp[3] = controlname;
        ...
        //			}
        //		}
        public static void openPage(string partId,string title)
{
            string id = partId;
            if (form.doActive(id))
            {
            ...
            string subpath = tree.getPath(tree.getSiteNodeByID(id));
            string path = globalConst.CurSite.Path + subpath;
            ...
            form.addEditor(path, title, id, name, ptype);
        }
        ...
        public static void setPartParamValue(ref XmlDocument doc, string PartID,string ParaName,string ParaValue)
        {
            XmlNodeList nodes = doc.SelectNodes("partxml/param");
            foreach (XmlNode node in nodes)
            {
                if (node.SelectSingleNode("name").InnerText == ParaName)
                {
                    node.SelectSingleNode("value").InnerText = ParaValue;
                    string sql = "update parts set partxml='" + str.Dot2DotDot(doc.OuterXml) + "' where id='" + PartID + "'";
                    globalConst.CurSite.SiteConn.execSql(sql);
                    return;
                }
            }
        }
        public static string getPartParamValue(string PartID,string ControlName,string PartName,string ParaName)
        {
            string sql = "select a.name,a.controlid,a.partxml,b.name from parts a,controls b where a.id='" + PartID + "' and a.controlid=b.id";
            SqliteDataReader rdr = globalConst.CurSite.SiteConn.OpenRecord(sql);
            if (rdr.Read())
            {
                string _PartName = rdr.GetString(0);
                string WareID = rdr.GetString(1);
                string PartXml = rdr.GetString(2);
                string WareName = rdr.GetString(3);
                rdr.Close();
                if (WareName.Equals(ControlName) && _PartName.Equals(PartName))
                {
                    ...
                    {
                        if (node.SelectSingleNode("name").InnerText.Equals(ParaName))
                        {
                            curnode = node;
                            break;
                        }
                    }
                    if(curnode!=null)return curnode.SelectSingleNode("value").InnerText;
                }
            }
            else
            {
        ...
        public static string getPartXml(string PartID)
        {
            string sql = "select a.name,a.controlid,a.partxml,b.name from parts a,controls b where a.id='" + PartID + "' and a.controlid=b.id";
        ...
        public static void setPartParamValue(string PartID, string ControlName, string PartName, string ParaName,string Value)
        {
            string sql = "select a.name,a.controlid,a.partxml,b.name from parts a,controls b where a.id='" + PartID + "' and a.controlid=b.id";
            SqliteDataReader rdr = globalConst.CurSite.SiteConn.OpenRecord(sql);
            if (rdr.Read())
            {
                ...
                if (WareName.Equals(ControlName) && _PartName.Equals(PartName))
                {
                    ...
                    if (curnode != null)
                    {
                        curnode.SelectSingleNode("value").InnerText = Value;
        ...
        public static string getPartHtml(string WareID, string WareName, string PartName, string PartXml, int AsPortal)
        {
            ...
            try
            {
                ...
                PartHtml = "<label style='color:blue' class='parthead'>" + InstanceName + "</label>" + PartHtml;
                return PartHtml;
            }
            catch (Exception ex)
            {
        ...
        public static void doHtmlAdapter(mshtml.IHTMLElement ele)
        {
            ...
            if (PageWare.isFormElement(ele) && globalConst.MdiForm.CurPropTag.SelectedIndex==0)
            {
                if (globalConst.FormDataMode)
                {
                    ...
                    {
                        case "label":
                            FTDPClient.PropertySpace.FormElementData.Label.curEle = ele;
                            globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Label.Bag();
                            break;
                        case "input":
                            switch (Adapter.Property.PropertyAdapter.getEleAttr(ele, "type").ToLower())
                            {
                                case "text":
                                    eleid = FTDPClient.PropertySpace.FormElementData.func.getID(ele);
                                    ...
                                    {
                                        FTDPClient.PropertySpace.FormElementData.TextBox.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.TextBox.Bag();
                                    }
                                    break;
                                case "password":
                                    FTDPClient.PropertySpace.FormElementData.Password.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Password.Bag();
                                    break;
                                case "button":
                                    eleid = FTDPClient.PropertySpace.FormElementData.func.getID(ele);
                                    if (eleid.Equals("ftform_flowaddaction"))
                                    {
                                        FTDPClient.PropertySpace.FormElementData.Flow.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Flow.Bag();
                                    }
                                    ...
                                    {
                                        FTDPClient.PropertySpace.FormElementData.FlowExt1.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.FlowExt1.Bag();
                                    }
                                    else if (eleid.Equals("ftform_flowext2action"))
                                    {
                                        FTDPClient.PropertySpace.FormElementData.FlowExt2.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.FlowExt2.Bag();
                                    }
                                    else if (eleid.Equals("ftform_rowrate"))
                                    {
                                        FTDPClient.PropertySpace.FormElementData.RowRate.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.RowRate.Bag();
                                    }
                                    else
                                    {
                                        AdapterProperty(null);
                                ...
                                case "reset":
                                    ...
                                    break;
                                case "checkbox":
                                    FTDPClient.PropertySpace.FormElementData.Checkbox.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Checkbox.Bag();
                                    break;
                                case "radio":
                                    FTDPClient.PropertySpace.FormElementData.Radio.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Radio.Bag();
                                    break;
                                case "file":
                                    FTDPClient.PropertySpace.FormElementData.FileBox.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.FileBox.Bag();
                                    break;
                                default:
                                    AdapterProperty(null);
                                    break;
                            ...
                            break;
                        case "textarea":
                            FTDPClient.PropertySpace.FormElementData.TextArea.curEle = ele;
                            globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.TextArea.Bag();
                            break;
                        case "select":
                            eleid = FTDPClient.PropertySpace.FormElementData.func.getID(ele);
                            if (eleid.Equals("ftform_category"))
                            {
                                FTDPClient.PropertySpace.FormElementData.Category.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Category.Bag();
                            }
                            else if (eleid.Equals("ftform_dateyear"))
                            {
                                FTDPClient.PropertySpace.FormElementData.DateAll.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.DateAll.Bag();
                            }
                            ...
                            {
                                FTDPClient.PropertySpace.FormElementData.StatFilter.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.StatFilter.Bag();
                            }
                            else
                            {
                                FTDPClient.PropertySpace.FormElementData.Select.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.Select.Bag();
                            }
                            break;
                        case "span":
                            FTDPClient.PropertySpace.FormElementData.RowIndex.curEle = ele;
                            globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElementData.RowIndex.Bag();
                            break;
                    }
                }
                else
                {
                    ...
                    {
                        case "label":
                            FTDPClient.PropertySpace.FormElement.Label.curEle = ele;
                            globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Label.Bag();
                            break;
                        case "input":
                            switch (Adapter.Property.PropertyAdapter.getEleAttr(ele, "type").ToLower())
                            {
                                case "text":
                                    eleid = FTDPClient.PropertySpace.FormElement.func.getID(ele);
                                    if (eleid.Equals("ftform_memberinput"))
                                    {
                                        AdapterProperty(null);
                                    }
                                    ...
                                    {
                                        FTDPClient.PropertySpace.FormElement.TextBox.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.TextBox.Bag();
                                    }
                                    break;
                                case "password":
                                    FTDPClient.PropertySpace.FormElement.Password.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Password.Bag();
                                    break;
                                case "button":
                                    eleid = FTDPClient.PropertySpace.FormElement.func.getID(ele);
                                    if (eleid.Equals("ftform_flowaddaction"))
                                    {
                                        FTDPClient.PropertySpace.FormElement.Flow.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Flow.Bag();
                                    }
                                    else if (eleid.Equals("ftform_flowmodaction"))
                                    {
                                        FTDPClient.PropertySpace.FormElement.Flow.curEle = ele;
                                    ...
                                    {
                                        FTDPClient.PropertySpace.FormElement.FlowExt1.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.FlowExt1.Bag();
                                    }
                                    else if (eleid.Equals("ftform_flowext2action"))
                                    {
                                        FTDPClient.PropertySpace.FormElement.FlowExt2.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.FlowExt2.Bag();
                                    }
                                    else if (eleid.Equals("ftform_rowrate"))
                                    {
                                        FTDPClient.PropertySpace.FormElement.RowRate.curEle = ele;
                                        globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.RowRate.Bag();
                                    }
                                    else
                                    {
                                        AdapterProperty(null);
                                ...
                                case "reset":
                                    ...
                                    break;
                                case "checkbox":
                                    FTDPClient.PropertySpace.FormElement.Checkbox.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Checkbox.Bag();
                                    break;
                                case "radio":
                                    FTDPClient.PropertySpace.FormElement.Radio.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Radio.Bag();
                                    break;
                                case "file":
                                    FTDPClient.PropertySpace.FormElement.FileBox.curEle = ele;
                                    globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.FileBox.Bag();
                                    break;
                                default:
                                    AdapterProperty(null);
                                    break;
                            ...
                            break;
                        case "textarea":
                            FTDPClient.PropertySpace.FormElement.TextArea.curEle = ele;
                            globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.TextArea.Bag();
                            break;
                        case "select":
                            eleid = FTDPClient.PropertySpace.FormElement.func.getID(ele);
                            if (eleid.Equals("ftform_category"))
                            {
                                FTDPClient.PropertySpace.FormElement.Category.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Category.Bag();
                            }
                            else if (eleid.Equals("ftform_dateyear"))
                            {
                                FTDPClient.PropertySpace.FormElement.DateAll.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.DateAll.Bag();
                            }
                            else if (eleid.Equals("ftform_datemonth"))
                            {
                                FTDPClient.PropertySpace.FormElement.DateAll.curEle = ele;
                            ...
                            {
                                FTDPClient.PropertySpace.FormElement.StatFilter.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.StatFilter.Bag();
                            }
                            else
                            {
                                FTDPClient.PropertySpace.FormElement.Select.curEle = ele;
                                globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.Select.Bag();
                            }
                            break;
                        case "span":
                            FTDPClient.PropertySpace.FormElement.RowIndex.curEle = ele;
                            globalConst.MdiForm.PropGrid.SelectedObject = FTDPClient.PropertySpace.FormElement.RowIndex.Bag();
                            break;
                    }
                }
                return;
        ...
        private static void doPartProperty(mshtml.IHTMLElement ele)
        {
            ...
            SqliteDataReader rdr2 = globalConst.ConfigConn.OpenRecord(sql);
            if (rdr2.Read())
            {
            ...
            SqliteDataReader rdr3 = globalConst.ConfigConn.OpenRecord(sql1);
            if (rdr3.Read())
            {
            ...
            SqliteDataReader rdr4 = globalConst.CurSite.SiteConn.OpenRecord("select * from controls where id='" + ControlID + "'");
            if (rdr4.Read())
            {
            ...
            foreach (XmlNode nd in nds)
            {
                ...
                ExitFor:
                    ...
                    {
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(TableSelectEditor), typeof(TableSelectEditorConverter)));
                    }
                    ...
                    else if (CusEditorTag.StartsWith("list-SearchDefine"))
                    {
                        ...
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(CommonSelColsEditorType1), typeof(CommonSelColsEditorConverter)));
                    }
                    ...
                    else if (CusEditorTag.StartsWith("list-OrderBy"))
                    {
                        ...
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(CommonSelColsEditorType2), typeof(CommonSelColsEditorConverter)));
                    }
                    ...
                    else if (CusEditorTag.Equals("list-MenuButtonSet"))
                    {
                        ...
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(ListMenuEditor), typeof(ListMenuEditorConverter)));
                    }
                    else if (CusEditorTag.Equals("list-APISet"))
                    {
                        ListAPIEditor.PartID = partid;
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(ListAPIEditor), typeof(ListAPIEditorConverter)));
                    }
                    else if (CusEditorTag.Equals("dyvalue-APISet"))
                    {
                        DyValueAPIEditor.PartID = partid;
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(DyValueAPIEditor), typeof(DyValueAPIEditorConverter)));
                    }
                    else if (CusEditorTag.Equals("dataop-APISet"))
                    {
                        DataOPAPIEditor.PartID = partid;
                        bag.Properties.Add(new PropertySpec(_caption, typeof(string), _category, _description, _value, typeof(DataOPAPIEditor), typeof(DataOPAPIEditorConverter)));
                    }
                    ...
                    else
                    {
                        switch (_class)
                        {
                            ...
                            case "string|image":
                                ...
                                break;
                            case "color":
                                bag.Properties.Add(new PropertySpec(_caption, typeof(System.Drawing.Color), _category, _description, ColorTranslator.FromHtml(_value)));
                                break;
                            case "int":
            ...
            FTDPClient.Adapter.PartImage.thisURL = form.getEditor().thisUrl;
            FTDPClient.Adapter.PartImage.thisName = form.getEditor().thisName;
            FTDPClient.Adapter.PartImage.thisPartID = partid;
        ...
        ExitFor:
            ...
            switch (_class)
            {
                ...
                case "string|image":
                    ...
                    break;
                case "color":
                    setNode.SelectSingleNode("value").InnerText = ColorTranslator.ToHtml((System.Drawing.Color)pValue);
