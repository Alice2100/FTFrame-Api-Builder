...
namespace FTDPClient.PropertySpace.Site
{
    ...
    public class PropertySite
    {
        ...
        public static int getSiteInt32(string name)
        {
            System.ComponentModel.LicenseManager.Validate(typeof(FTDPClient.forms.NewControl));
            string sql = "select " + name + " from sites where id='" + globalConst.CurSite.ID + "'";
            return globalConst.ConfigConn.GetInt32NoNull(sql);
        }
        ...
        public static (string NetPubPath,string JavaPubPath) GetPubPath()
        {
            var netPath = "";
            var javaPath = "";
            var setStr=getSiteString("publocalroot");
            if(!string.IsNullOrEmpty(setStr))
            {
                var items = setStr.Split(new string[] { "[##]" }, StringSplitOptions.None);
                if (items.Length > 0) netPath = items[0];
                if (items.Length > 1) javaPath = items[1];
            }
            return (netPath, javaPath);
        }
        private static void SetNetPubPath(string _value)
        {
            if (_value.ToLower() == "c:\\" || _value.ToLower() == "d:\\") _value = "";
            var oriVal = GetPubPath();
            setSiteString("publocalroot",_value+"[##]"+ oriVal.JavaPubPath);
        }
        private static void SetJavaPubPath(string _value)
        {
            if (_value.ToLower() == "c:\\" || _value.ToLower() == "d:\\") _value = "";
            var oriVal = GetPubPath();
            setSiteString("publocalroot", oriVal.NetPubPath + "[##]" + _value);
        }
        ...
        public static PropertyBag Bag()
        {
            ...
            var pubPathVal = GetPubPath();
            bag.Properties.Add(new PropertySpec(res._propertysite.GetString("ad_localroot"), typeof(string), adcate, res._propertysite.GetString("ad_localroot_desc"), pubPathVal.NetPubPath, typeof(SelectFolderEditor), typeof(SelectFolderEditorConverter)));
            bag.Properties.Add(new PropertySpec(res._propertysite.GetString("ad_localroot_java"), typeof(string), adcate, res._propertysite.GetString("ad_localroot_desc"), pubPathVal.JavaPubPath, typeof(SelectFolderEditor), typeof(SelectFolderEditorConverter)));
            return bag;
        }
        ...
        private static void site_GetValue(object sender, PropertySpecEventArgs e)
        {
            ...
            switch (pName)
            {
                ...
                default:
                    ...
                    if (pName.Equals(res._propertysite.GetString("ad_localroot")))
                    {
                        e.Value = GetPubPath().NetPubPath;
                        return;
                    }
                    if (pName.Equals(res._propertysite.GetString("ad_localroot_java")))
                    {
                        e.Value = GetPubPath().JavaPubPath;
                        return;
                    }
                    break;
            }
        ...
        private static void site_SetValue(object sender, PropertySpecEventArgs e)
        {
            ...
            switch (pName)
            {
                case "Domin":
                    ...
                    {
                        globalConst.MdiForm.SiteTree.SelectedNode.Text = globalConst.CurSite.SiteTreeRootCaption;
                        ((string[])(globalConst.MdiForm.SiteTree.SelectedNode.Tag))[0] = globalConst.CurSite.SiteTreeRootCaption;
                        ((string[])(globalConst.MdiForm.SiteTree.SelectedNode.Tag))[1] = globalConst.CurSite.SiteTreeRootCaption;
                    }
                    break;
                case "Caption":
                ...
                default:
                    ...
                    if (pName.Equals(res._propertysite.GetString("ad_localroot")))
                    {
                        SetNetPubPath(pValue);
                        return;
                    }
                    if (pName.Equals(res._propertysite.GetString("ad_localroot_java")))
                    {
                        SetJavaPubPath(pValue);
                        return;
                    }
                    break;
            }
    ...
    public class PropertyPage
    {
        ...
        public static PropertyBag Bag()
        {
            ...
            string fore_frame = getPageString("fore_frame");
            if (string.IsNullOrWhiteSpace(fore_frame)) fore_frame = "JQueryUI";
            string out_type = getPageString("out_type");
            if (string.IsNullOrWhiteSpace(out_type)) out_type = "Page";
            bag.Properties.Add(new PropertySpec(res.@class.str("Class.4"), typeof(string), res.@class.str("Class.5"), res.@class.str("Class.6"), getPageJS(), typeof(classes.TextEditor), typeof(TextEditorConverter)));
            bag.Properties.Add(new PropertySpec(res.@class.str("Class.9"), typeof(string), res.@class.str("Class.5"), res.@class.str("Class.7"), fore_frame, typeof(System.Drawing.Design.UITypeEditor), typeof(ForeFrameConverter)));
            bag.Properties.Add(new PropertySpec(res.@class.str("Class.10"), typeof(string), res.@class.str("Class.5"), res.@class.str("Class.8"), out_type, typeof(System.Drawing.Design.UITypeEditor), typeof(OutTypeConverter)));
            return bag;
        }
        ...
        private static void page_GetValue(object sender, PropertySpecEventArgs e)
        {
            ...
            switch (pName)
            {
                ...
                case "UpdateTime":
                    ...
                    break;
                default:
                    if (pName.Equals(res.@class.str("Class.4")))
                    {
                        e.Value = getPageString("pagejs");
                        return;
                    }
                    else if (pName.Equals(res.@class.str("Class.9")))
                    {
                        string fore_frame = getPageString("fore_frame");
                        if (string.IsNullOrWhiteSpace(fore_frame)) fore_frame = "JQueryUI";
                        e.Value= fore_frame;
                    }
                    else if (pName.Equals(res.@class.str("Class.10")))
                    {
                        string out_type = getPageString("out_type");
                        if (string.IsNullOrWhiteSpace(out_type)) out_type = "Page";
                        e.Value = out_type;
        ...
        private static void page_SetValue(object sender, PropertySpecEventArgs e)
        {
            ...
            switch (pName)
            {
                ...
                case "Caption":
                    ...
                    break;
                default:
                    if(pName== res.@class.str("Class.4"))
                    {
                        setPageString("pagejs", pValue);
                    }
                    else if(pName== res.@class.str("Class.9"))
                    {
                        setPageString("fore_frame", pValue);
                    }
                    else if (pName == res.@class.str("Class.10"))
                    {
                        setPageString("out_type", pValue);
                        TreeNode nd = tree.getSiteNodeByID(pageid);
                        if (pValue == PageOutType.Page.ToString()) { nd.ImageIndex = 19; nd.SelectedImageIndex = 19; }
                        else if (pValue == PageOutType.Json.ToString()) { nd.ImageIndex = 11; nd.SelectedImageIndex = 11; }
                    }
                    //case "页面JS":
                    //    setPageString("pagejs", pValue);
                    /*
    ...
    public class PropertyPart
    {
        ...
        public static void doPartProperty(string id)
        {
            ...
            if (globalConst.MdiForm.SiteTree.SelectedNode == null)
            {
                ...
                {
                    FTDPClient.Adapter.PartImage.thisURL = "";
                    FTDPClient.Adapter.PartImage.thisName = "";
                ...
                else
                {
                    ...
                    FTDPClient.Adapter.PartImage.thisURL = globalConst.CurSite.Path + tree.getPath(cnd.Parent);
                    FTDPClient.Adapter.PartImage.thisName = ((string[])(cnd.Parent.Tag))[0];
                    FTDPClient.Adapter.PartImage.thisPartID = partid;
                }
            }
            else
            {
                FTDPClient.Adapter.PartImage.thisURL = globalConst.CurSite.Path + tree.getPath(globalConst.MdiForm.SiteTree.SelectedNode.Parent);
                if(globalConst.MdiForm.SiteTree.SelectedNode.Parent.Tag!=null) FTDPClient.Adapter.PartImage.thisName = ((string[])(globalConst.MdiForm.SiteTree.SelectedNode.Parent.Tag))[0];
                FTDPClient.Adapter.PartImage.thisPartID = partid;
            }
