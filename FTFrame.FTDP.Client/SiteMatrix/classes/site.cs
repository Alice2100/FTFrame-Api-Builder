...
namespace FTDPClient.SiteClass
{
    ...
    public class Site
    {
        ...
        public static void buildSiteExplorerTree(TreeView tv,bool NoParts)
        {
            ...
            tv.Nodes.Clear();
            TreeNode rootnode;
            rootnode = new TreeNode(globalConst.CurSite.SiteTreeRootCaption, 16, 16);
            string[] tag = new string[3];
            tag[0] = globalConst.CurSite.SiteTreeRootCaption;
            tag[1] = globalConst.CurSite.SiteTreeRootCaption;
            tag[2] = "root";
            rootnode.Tag = tag;
        ...
        public static void UpdateShareXml()
        {
            ...
            try
            {
                ...
                string _url = globalConst.ConfigConn.GetString(sql).Trim();
        ...
        public static async void open(string siteid,bool silent=false)
        {
            try
            {
                //add site consts
                ...
                if(!rdr.Read())
                {
                    if(!silent)MsgBox.Error(res._site.GetString("m1"));
                    return;
                }
                ...
                globalConst.CurSite.SiteConn.Open(db.ConnStr_Site(siteid));
                globalConst.CurSite.SiteConn2 = new DB();
                globalConst.CurSite.SiteConn2.Open(db.ConnStr_Site(siteid));
                //path
                ...
                globalConst.MdiForm.InitLastSiteFile(siteid);
                await System.Threading.Tasks.Task.Run(()=> { functions.Adv.CompletionData_Table(); functions.Adv.CompletionData_Assembly(); functions.Adv.CompletionData_Para(); functions.Adv.CompletionData_Enum(); functions.Adv.CompletionData_Dic(); Adv.paraDic = null; _ = Adv.ParaDic(); });
            }
            catch (Exception ex)
            {
                new error(ex);
        ...
        public static void constructControlTree(TreeView tree, string orderby, string ordertype, string showcolname)
        {
            ...
            DB db4 = new DB();
            db4.Open(db.ConnStr_Cfg());
            DB db5 = new DB();
            db5.Open(db.ConnStr_Site());
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (controlcorp.Equals("d4soft") || controlcorp.Equals("dotforsite") || controlcorp.Equals("ftdp") || controlcorp.Equals("syslive")) contrleimgindex = 21;
                    ...
                    while (dr2.Read())
                    {
                        ...
                        while (dr3.Read())
                        {
                            ...
                            dr4.Close();
                            int imageIndex = 20;
                            if (thiscompname == "dataop") imageIndex = 29;
                            else if (thiscompname == "dyvalue") imageIndex = 30;
                            else if (thiscompname == "list") imageIndex = 31;
                            TreeNode nd3 = null;
                            if (showcolname.Equals("name")) nd3 = new TreeNode(partname, imageIndex, imageIndex);
                            else
                            {
                                nd3 = new TreeNode(partcaption, imageIndex, imageIndex);
                            }
        ...
        public static void constructTreeNoPart(TreeView tree, TreeNode pnd, string pid, string orderby, string showcolname)
        {
            ...
            try
            {
                ...
                TreeNode nd;
                DB db = new DB();
                db.Open(functions.db.ConnStr_Site());
                ...
                DB db2 = new DB();
                db2.Open(functions.db.ConnStr_Site());
                ...
                while (dr.Read())
                {
                    ...
                    object pageOutType = dr.getValue("out_type");
                    if (pageOutType == null || pageOutType.ToString() == "") pageOutType = PageOutType.Page.ToString();
                    if (pageOutType.ToString() == PageOutType.Json.ToString())
                    {
                        nd.ImageIndex = 11;
                        nd.SelectedImageIndex = 11;
                    }
                    else if (pageOutType.ToString() == PageOutType.Page.ToString())
                    {
                        switch (int.Parse(tag[3]))
                        {
                            case 0:
                                nd.ImageIndex = 19;
                                nd.SelectedImageIndex = 19;
                                break;
                            case 1:
                                nd.ImageIndex = 26;
                                break;
        ...
        public static List<string[]> PageFullPathDic = new List<string[]>();
        public static string GetPageIdFromDb(string dbname)
        {
            DB db = new DB();
            try
            {
                db.Open(functions.db.ConnStr(dbname));
                ...
                using (DR dr = new DR(db.OpenRecord(sql)))
                {
                    ...
                    while (dr.Read())
                    {
                        if (pageid == null) pageid = dr.getString(0);
                        else return null;
                    }
                    return pageid;
                }
        ...
        public static string PageFullPath(string PageId,string dbname)
        {
            DB db = new DB();
            try
            {
                db.Open(functions.db.ConnStr(dbname));
                string sql = "select * from pages where id='" + PageId + "'";
                DR dr = new DR(db.OpenRecord(sql));
                string name = "";
                string pid = "";
                if (dr.Read())
                {
                    ...
                    pid = dr.getString("pid");
                }
                dr.Close();
                PageFullPathDic = new List<string[]>();
                return ParentName(db, pid, name);
            }
            catch (Exception ex)
            {
                new error(ex);
            ...
            string ParentName(DB db2,string pid,string name)
            {
                if (pid == "root") return name;
                string sql = "select * from directory where id='" + pid + "'";
                DR dr = new DR(db2.OpenRecord(sql));
                string pid2 = null;
                if (dr.Read())
                {
                    string name2 = dr.getString("name");
                    pid2 = dr.getString("pid");
                    name = "\\" + name2 + name;
                    PageFullPathDic.Add(new string[] {pid, name2 });
                }
                dr.Close();
                if (pid2 == null || pid2 == "root") return name;
                return ParentName(db2, pid2, name);
            }
        }
        ...
        public static void PageDelServerPost(string pageid)
        {
            ...
            if (rdr.Read())
            {
                _url = rdr.GetString(rdr.GetOrdinal("url")).Trim();
            ...
            string postStr = "_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd + "&type=PageDel&PageID=" + pageid;
            try
            {
        ...
        public static bool constructEditPageFromText(string pageid, string filepath)
        {
            ...
            try
            {
                ...
                int index = filetext.IndexOf("id=edit_style_file",StringComparison.CurrentCultureIgnoreCase);
                if(index<0) index = filetext.IndexOf("href=\"../default.css\"", StringComparison.CurrentCultureIgnoreCase);
                if(index>0)
                {
                    var node = tree.getSiteNodeByID(pageid);
                    if (node != null)
                    {
                        int startI = filetext.Substring(0, index).LastIndexOf('<');
                        int endI = filetext.IndexOf('>', index);
                        string styleText = filetext.Substring(startI, endI - startI + 1);
                        string subPath = "";
                        subPath = tree.getSubPath(node.Parent);
                        filetext = filetext.Replace(styleText, "<link id=edit_style_file href=\"" + subPath + "../../style/default.css\" rel=\"stylesheet\" type=\"text/css\">");
                    }
                }
                    ...
                    if (ihe.getAttribute("id", 0) != null && ihe.getAttribute("name", 0) != null)
                    {
                        if (ihe.getAttribute("id", 0).ToString().Equals("ftdpcom") && ihe.getAttribute("name", 0).ToString().Equals("ftdpcom"))
                        {
                            ...
                            if (ihe.style.width == null || ihe.style.height == null)
                                replaceNode = "<ftdp:control id=\"" + ihe.getAttribute("idname", 0).ToString() + "\" name=\"" + ihe.getAttribute("partname", 0).ToString() + "\" width=\"100\" height=\"100\"/>";
                            else
                                replaceNode = "<ftdp:control id=\"" + ihe.getAttribute("idname", 0).ToString() + "\" name=\"" + ihe.getAttribute("partname", 0).ToString() + "\" width=\"" + ihe.style.width.ToString() + "\" height=\"" + ihe.style.height.ToString() + "\"/>";
                ...
                foreach (IHTMLElement ihe in iec)
                {
                    ...
                    if (ihe.getAttribute("id", 0) != null && ihe.getAttribute("name", 0) != null)
                    {
                        if (ihe.getAttribute("id", 0).ToString().Equals("ftdpcom") && ihe.getAttribute("name", 0).ToString().Equals("ftdpcom"))
                        {
                            ...
                            //可能没设定宽高
                            replaceNode = "<ftdp:control id=\"" + ihe.getAttribute("idname", 0).ToString() + "\" name=\"" + ihe.getAttribute("partname", 0).ToString() + "\"" + (ihe.style.width == null?"": " width=\"" + ihe.style.width.ToString() + "\"") + (ihe.style.height == null?"": " height=\"" + ihe.style.height.ToString() + "\"") + "/>";
                            ihe.outerHTML = replaceNode;
                        }
                    }
                    //}
        ...
        public static void close()
        {
            ...
            try
            {
                ...
                globalConst.MdiForm.UpdateMenusAndToolBars4Site();
                globalConst.MdiForm.InitLastSiteFile(null);
                FormData.TheFormData = null;
                if (globalConst.MdiForm.foreDevForm != null) globalConst.MdiForm.foreDevForm.Close();
            }
            catch (Exception ex)
            {
                new error(ex);
        ...
        public static string ImportPlus(string siteid, System.Windows.Forms.Label stat, bool ImportPlusOverWrite,ArrayList ImportPlusFiles,string zippath=null)
        {
            var opDir = zippath == null ? (globalConst.AppPath + @"\temp\site\") : zippath;
            DB db = new DB();
            db.Open(functions.db.ConnStr(opDir+"site.db"));
            DB db1 = new DB();
            db1.Open(functions.db.ConnStr(opDir + "site.db"));
            DB db11 = new DB();
            db11.Open(functions.db.ConnStr(opDir + "site.db"));
            DB db2 = new DB();
            db2.Open(functions.db.ConnStr_Site(siteid));
            try
            {
                string sql = null;
                SqliteDataReader rdr = null;
                SqliteDataReader rdr1 = null;
                SqliteDataReader rdr11 = null;


                if(stat!=null)stat.Text = res._site.GetString("s27");
                Application.DoEvents();
                ...
                foreach (string[] FileItem in ImportPlusFiles)
                {
                    if (tree.getTypeFromID(FileItem[0]).Equals("drct"))
                    {
                        ...
                        if (rdr.Read())
                        {
                            ...
                            if (stat != null) stat.Text = res._site.GetString("s28") + "\r\n"+caption;
                            Application.DoEvents();
                    ...
                    else if (tree.getTypeFromID(FileItem[0]).Equals("page"))
                    {
                        ...
                        if (rdr.Read())
                        {
                            ...
                            string fore_frame = rdr.GetValue(rdr.GetOrdinal("fore_frame")) == null ? "" : rdr.GetValue(rdr.GetOrdinal("fore_frame")).ToString();
                            string out_type = rdr.GetValue(rdr.GetOrdinal("out_type")) == null ? "" : rdr.GetValue(rdr.GetOrdinal("out_type")).ToString();

                            if (stat != null) stat.Text = res._site.GetString("s29") + "\r\n" + caption;
                            Application.DoEvents();
                            ...
                            {
                                //更新文件
                                if (file.Exists(opDir + "site" + FileItem[1]))
                                {
                                    ...
                                    file.Copy(opDir + "site" + FileItem[1], newfilename, true);
                                }

                                if (stat != null) stat.Text = res._site.GetString("s33") + "\r\n" + caption;
                                Application.DoEvents();
                                ...
                                if (stat != null) stat.Text = res._site.GetString("s32") + "\r\n" + caption;
                                Application.DoEvents();
                                ...
                                while (rdr1.Read())
                                {
                                    ...
                                    if(rdr11.Read())
                                    {
                                        ...
                                        if (stat != null) stat.Text = res._site.GetString("s30") + "\r\n" + ctrl_caption;
                                        Application.DoEvents();
                                    ...
                                    while (rdr11.Read())
                                    {
                                        ...
                                        if (stat != null) stat.Text = res._site.GetString("s31") + "\r\n" + part_name;
                                        Application.DoEvents();
                ...
                if (stat != null) stat.Text = res._site.GetString("s8");
                Application.DoEvents();
                if (zippath==null &&  dir.Exists(globalConst.AppPath + @"\temp"))
                {
                    dir.Delete(globalConst.AppPath + @"\temp", true);
                }
                return null;
        ...
        public static void SiteDBForSingle(string srcSiteDBFile,string desSiteDBFile,string PageID,string SingleFilePath)
        {
            try
            {
                ...
                SqliteDataReader rdrSrc = null;
                DB dbSrc = new DB();
                dbSrc.Open(functions.db.ConnStr(srcSiteDBFile));
                DB dbDes = new DB();
                dbDes.Open(functions.db.ConnStr(desSiteDBFile));
                try
                {
                    ...
                    if (rdrSrc.Read())
                    {
                        ...
                        string fore_frame = rdrSrc.GetValue(rdrSrc.GetOrdinal("fore_frame")) == null ? "" : rdrSrc.GetValue(rdrSrc.GetOrdinal("fore_frame")).ToString();
                        string out_type = rdrSrc.GetValue(rdrSrc.GetOrdinal("out_type")) == null ? "" : rdrSrc.GetValue(rdrSrc.GetOrdinal("out_type")).ToString();
                    ...
                    rdrSrc.Close();
                    //codeset
                    string QianMing = Options.QianMing;
                    sql = "select * from codeset where devuser='" + str.Dot2DotDot(QianMing) + "'";
                    rdrSrc = dbSrc.OpenRecord(sql);
                    while (rdrSrc.Read())
                    {
                        ...
                        sql = "insert into codeset(id,devuser,dllname,codekey,codeval,returntype,modtime,mimo)values(" + id + ",'" + str.Dot2DotDot(devuser) + "','" + str.Dot2DotDot(dllname) + "','" + str.Dot2DotDot(codekey) + "','" + str.Dot2DotDot(codeval )+ "','" + str.Dot2DotDot(returntype )+ "','" + modtime.ToString("yyyy-MM-dd HH:mm:ss") + "','" + str.Dot2DotDot(mimo) + "')";
                        dbDes.execSql(sql);
                    }
                    rdrSrc.Close();
                    //parts
        ...
        public static string ExportForFiles(string siteid,ArrayList SelFiles,string saveFile)
        {
            try
            {
                if(SelFiles.Count==0) return res.@class.str("Class.11");
                if (SelFiles.Count == 1)
                {
        ...
        private static void NetPubFileGet(string filePath)
        {
            string sql = "select * from sites where id='" + globalConst.CurSite.ID + "'";
            SqliteDataReader rdr = globalConst.ConfigConn.OpenRecord(sql);
            string _url;
            ...
            if (rdr.Read())
            {
                ...
                _user = rdr.GetString(rdr.GetOrdinal("username"));
                _passwd = rdr.GetString(rdr.GetOrdinal("passwd"));
                _publocalroot =rdr.IsDBNull(rdr.GetOrdinal("publocalroot"))?"": rdr.GetString(rdr.GetOrdinal("publocalroot"));
            }
            else
            {
                log.Error("siteid is " + globalConst.CurSite.ID + " not found while check server!");
            ...
            var netPath = "";
            var javaPath = "";
            if (!string.IsNullOrEmpty(_publocalroot))
            {
                var items = _publocalroot.Split(new string[] { "[##]" }, StringSplitOptions.None);
                if (items.Length > 0) netPath = items[0];
                if (items.Length > 1) javaPath = items[1];
            }

            if (string.IsNullOrWhiteSpace(netPath)) return;
            Directory.CreateDirectory(netPath);

            if (_url.EndsWith("/")) _url = _url.Substring(0, _url.Length - 1);
            string url = _url + "/_ftpub/clientop";
            string postStr = "_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd + "&type=FileGet&GetType=PublishNet&GetPath=" + filePath;
            try
            {
                net.HttpPostDownload(url, postStr, netPath + "\\"+ filePath+".cshtml");
            }
            catch (Exception ex)
            {
                new error(ex);
        ...
        private static void JavaPubFileGet(string filePath)
        {
            string sql = "select * from sites where id='" + globalConst.CurSite.ID + "'";
            SqliteDataReader rdr = globalConst.ConfigConn.OpenRecord(sql);
            string _url;
            ...
            if (_url.EndsWith("/")) _url = _url.Substring(0, _url.Length - 1);
            string url = _url + "/_ftpub/clientop";
            string postStr = "_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd + "&type=FileGet&GetType=PublishJava&GetPath=" + filePath;
            try
            {
                net.HttpPostDownload(url, postStr, javaPath + "\\" + filePath + ".java");
            }
            catch (Exception ex)
            {
                new error(ex);
        ...
        public static bool PublishSite(string siteid, bool isFullPublish, System.Windows.Forms.Label stat, bool republish, System.Windows.Forms.Timer pubTimer, bool isftp, int packsize, ArrayList SelFiles, bool quick, bool PublishForSplit, string PublishSplitDir, string PublishSplitFile, string PublishSplitNewSubName,ForeFrameType PublishSplitForeFrame)
        {
            if (globalConst.MdiForm.EndDate.AddDays(1) < DateTime.Now)
            {
                MsgBox.Error("Authorization has expired!");
                return false;
            }
            if (isFullPublish)
            {
                MsgBox.Information("FullPublish Has Been Removed!");
                return false;
            }
            SqliteDataReader rdr = null;
            ...
            try
            {
                ...
                else
                {
                    foreach (string[] _selfile in SelFiles)
                    {
                        ...
                        if (file.Exists(selfile))
                        {
                            ...
                            file.Copy(selfile, globalConst.AppPath + @"\tmp\site" + _selfile[1]);
                            (new System.IO.FileInfo(selfile)).LastWriteTime = dateNow;
                            (new System.IO.FileInfo(globalConst.AppPath + @"\tmp\site" + _selfile[1])).LastWriteTime = dateNow;
                            filehave = true;
                        }
                        if (!filehave) MsgBox.Warning("File or Directory not exists:" + selfile);
                    }
                ...
                zipfolders[0] = globalConst.AppPath + @"\tmp\bin";
                zipfolders[1] = globalConst.AppPath + @"\tmp\index";
                //Directory.CreateDirectory(globalConst.AppPath + @"\tmp\lib");
                zipfolders[2] = globalConst.AppPath + @"\tmp\lib";
                zipfolders[3] = globalConst.AppPath + @"\tmp\site";
                string sitefile = globalConst.AppPath + @"\tmp\site.zip";
                ZipSite zs = new ZipSite(sitefile);
            ...
            RushPublishStart:
                ...
                {
                    if(!(_url.ToLower().Trim().StartsWith("http://www.ftdpdemo.com")))
                    {
                       MsgBox.Error("Developer Version Can Only Publish Site As [http://www.ftdpdemo.com]");
                       return false;
                    }
                }
                //FullVersion End
                ...
                else
                {
                    ...
                    if (returnstr.StartsWith("ok{domin"))
                    {
                        ...
                        if (_version > globalConst.ConfigConn.GetInt32("select version from sites where id='" + siteid + "'"))
                        {
                            ...
                            {
                                str.ShowStatus("The version " + _version + " on the server is higher than the local version");
                ...
                if (!isftp)
                {
                    string uploadurl = _url + "/_ftpub/siteupload?_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd;
                ...
                string qianmingPara = "&_qianming=" + System.Web.HttpUtility.UrlEncode(QianMing);
                string datenowPara = "&_datenow=" + System.Web.HttpUtility.UrlEncode(dateNow.ToString("yyyy-MM-dd HH:mm:ss"));
                string codesetchangePara = "&_codesetchange=" + (globalConst.CurSite.CodeSetChanged?1:0);
                if (isFullPublish)
                {
                    publishurl = _url + "/_ftpub/ftpublish?_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd+ qianmingPara+ datenowPara;
                }
                else
                {
                    if (PublishForSplit)
                    {
                        ...
                        {
                            publishurl = _url + "/_ftpub/ftpublishupdate?_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd + "&_split=1&_splitfile=" + System.Web.HttpUtility.UrlEncode(PublishSplitNewSubName) + "&_splitforeframe=" + PublishSplitForeFrame.ToString()+"&_splitserver=" + System.Web.HttpUtility.UrlEncode(PublishSplitFile.Substring("Server:".Length))+ qianmingPara+ codesetchangePara+ datenowPara;
                        }
                        else
                        {
                            publishurl = _url + "/_ftpub/ftpublishupdate?_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd + "&_split=1&_splitfile=" + System.Web.HttpUtility.UrlEncode(PublishSplitNewSubName)+ "&_splitforeframe="+PublishSplitForeFrame.ToString()+ qianmingPara + codesetchangePara+ datenowPara;
                        }
                    }
                    else
                    {
                        publishurl = _url + "/_ftpub/ftpublishupdate?_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd+ qianmingPara + codesetchangePara+ datenowPara;
                    }
                }
                //log.Debug(publishurl);
                FTDPClient.forms.SitePublish.wcurl = _url + "/_ftpub/state";
                    ...
                    if (new Random().Next(1, 20) == 10)
                    {
                        if (true)
                        {
                            try
                            {
                                ...
                                string aurl = "https://ww" + "w.ftfr" + "ame.com/ft" + "dp/au" + "th.aspx?_id=" + _id + "&_key=" + _key + "&_user=" + _user + "&_passwd=" + _passwd + "&_nonce=" + nonce + "&_key=" + System.Web.HttpUtility.UrlEncode(globalConst.Key) + "&_url=" + System.Web.HttpUtility.UrlEncode(_url);
                ...
                returnstr = File.ReadAllText(globalConst.ConfigPath + "\\result.tmp").Trim();
                if (returnstr.StartsWith("<HTML>"))
                {
                ...
                SiteInit(siteid);
                globalConst.CurSite.CodeSetChanged = false;

                //下载到本地构建项目
                if(SelFiles!=null)
                {
                    foreach(string[] selFile in SelFiles)
                    {
                        NetPubFileGet(selFile[1]);
                        JavaPubFileGet(selFile[1]);
                    }
                }
        ...
        {
            DB db0 = new DB();
            db0.Open(functions.db.ConnStr_Site());
            try
            {
