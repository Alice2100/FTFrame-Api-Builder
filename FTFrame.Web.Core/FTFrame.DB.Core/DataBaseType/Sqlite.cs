...
namespace FTFrame.DBClient.DataBaseType
{
    public class DBSqlite : DBInterface
    {
        ...
        {
            object obj = null;
            using (var cmd = new SqliteCommand(sqlstr, db))
            {
        ...
        {
            DR rdr = null;
            //using (SqliteCommand cmd = new SqliteCommand(sqlstr, db))
            //{
            //    cmd.CommandTimeout = commandTimeOut;
            //    rdr = new DR(new DRSqlite(cmd.ExecuteReader()), DataBase.Sqlite);
            //}
        ...
        {
            DR rdr = null;
            //using (SqliteCommand cmd = new SqliteCommand(sqlstr, db, ((SqliteTransaction)(st.st))))
            //{
            //    cmd.CommandTimeout = commandTimeOut;
            //    rdr = new DR(new DRSqlite(cmd.ExecuteReader()), DataBase.Sqlite);
            //}
            SqliteCommand cmd = new SqliteCommand(sqlstr, db, ((SqliteTransaction)(st.st)));
            {
                cmd.CommandTimeout = commandTimeOut;
                rdr = new DR(new DRSqlite(cmd.ExecuteReader()), DataBase.Sqlite);
            }
        ...
        public int ExecSql(string sqlstr, PR para, int commandTimeOut)
        {
            using (SqliteCommand cmd = new SqliteCommand(sqlstr, db))
            {
                cmd.Parameters.Add((SqliteParameter)para.pr);
                cmd.CommandTimeout = commandTimeOut;
                return cmd.ExecuteNonQuery();
            }
        }
        public int ExecSql(string sqlstr, IEnumerable<PR> paras, int commandTimeOut)
        {
            using (SqliteCommand cmd = new SqliteCommand(sqlstr, db))
            {
                foreach (PR para in paras) cmd.Parameters.Add((SqliteParameter)para.pr);
                cmd.CommandTimeout = commandTimeOut;
                return cmd.ExecuteNonQuery();
            }
        }
        ...
        {
            using (SqliteCommand cmd = new SqliteCommand(sqlstr, db, ((SqliteTransaction)(transaction.st))))
            {
                cmd.CommandTimeout = commandTimeOut;
                return cmd.ExecuteNonQuery();
            }
        ...
        {
            return new ST(db.BeginTransaction(IsolationLevel.ReadUncommitted), DataBase.Sqlite);
        }
        public ST GetTransaction(IsolationLevel isolationLevel)
        {
            return new ST(db.BeginTransaction(isolationLevel), DataBase.Sqlite);
        }
        public void Close()
        {
            db.Close();
    ...
    public class DRSqlite : DRInterface
    {
        private SqliteDataReader dr;
        public DRSqlite(SqliteDataReader reader)
        {
            dr = reader;
        }
