...
{
    public class DBOracle : DBInterface
    {
        OracleConnection db;
        ...
        {
            object obj = null;
            using (var cmd = new OracleCommand(sqlstr, db))
            {
        ...
        public DataTable GetDataTable(string sqlstr)
        {
            ...
            new OracleDataAdapter(sqlstr, db).Fill(dt);
            return dt;
        }
        public DataSet GetDataSet(string sql, int StartRecord, int MaxRecord, string SrcTable)
        {
            ...
            {
                new OracleDataAdapter(sql, db).Fill(ds);
            }
            else if (MaxRecord == -1 && SrcTable != null)
            {
                new OracleDataAdapter(sql, db).Fill(ds, SrcTable);
            }
            else
            {
                new OracleDataAdapter(sql, db).Fill(ds, StartRecord, MaxRecord, SrcTable);
            }
            return ds;
        }
        ...
        {
            DR rdr = null;
            using (OracleCommand cmd = new OracleCommand(sqlstr, db))
            {
                cmd.CommandTimeout = commandTimeOut;
                rdr = new DR(new DROracle(cmd.ExecuteReader()), DataBase.Oracle);
            }
            return rdr;
        }
        public DR OpenRecord(string sqlstr, ST st, int commandTimeOut)
        {
            ...
            {
                cmd.CommandTimeout = commandTimeOut;
                cmd.Transaction = ((OracleTransaction)(st.st));
                rdr = new DR(new DROracle(cmd.ExecuteReader()), DataBase.Oracle);
            }
            return rdr;
        }
        ...
        public int ExecSql(string sqlstr, PR para, int commandTimeOut)
        {
            using (OracleCommand cmd = new OracleCommand(sqlstr, db))
            {
                cmd.Parameters.Add((OracleParameter)para.pr);
                cmd.CommandTimeout = commandTimeOut;
                return cmd.ExecuteNonQuery();
            }
        }
        public int ExecSql(string sqlstr, IEnumerable<PR> paras, int commandTimeOut)
        {
            using (OracleCommand cmd = new OracleCommand(sqlstr, db))
            {
                foreach (PR para in paras) cmd.Parameters.Add((OracleParameter)para.pr);
                cmd.CommandTimeout = commandTimeOut;
                return cmd.ExecuteNonQuery();
            }
        }
        public int ExecSql(string sqlstr, ST transaction, int commandTimeOut)
        {
            ...
            {
                cmd.CommandTimeout = commandTimeOut;
                cmd.Transaction = ((OracleTransaction)(transaction.st));
                return cmd.ExecuteNonQuery();
            }
        }
        ...
        {
            return new ST(db.BeginTransaction(IsolationLevel.ReadUncommitted), DataBase.Oracle);
        }
        public ST GetTransaction(IsolationLevel isolationLevel)
        {
            return new ST(db.BeginTransaction(isolationLevel), DataBase.Oracle);
        }
        public void Close()
        {
            db.Close();
    ...
    public class DROracle : DRInterface
    {
        private OracleDataReader dr;
        public DROracle(OracleDataReader reader)
        {
            dr = reader;
        }
