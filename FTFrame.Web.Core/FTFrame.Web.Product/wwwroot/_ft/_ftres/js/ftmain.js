...
function StyleInit(elet) {
	(elet == null ? $("._fld") : $(elet).find("._fld")).hover(function () {
		$(this).css("background-color", "#fafafa");
	}
		, function () {
			$(this).css("background-color", "#ffffff");
		});
	(elet == null ? $("._input") : $(elet).find("._input")).focus(function () {
		$(this).css("background", "#F3FFC4").blur(function () {
			$(this).css("background", "#ffffff");
		});
	});
	(elet == null ? $("._select") : $(elet).find("._select")).focus(function () {
		$(this).css("background", "#F3FFC4").blur(function () {
			$(this).css("background", "#F8F8F8");
		});
	});
	(elet == null ? $(".tip") : $(elet).find(".tip")).tooltip({
		content: function () {
			var element = $(this);
			if (element.is("[title]")) {
	...
	(elet == null ? $("._tabs") : $(elet).find("._tabs")).tabs();

	(elet == null ? $("._tabs_left") : $(elet).find("._tabs_left")).tabs().addClass("ui-tabs-vertical ui-helper-clearfix");
	(elet == null ? $("._tabs_left li") : $(elet).find("._tabs_left li")).removeClass("ui-corner-top").addClass("ui-corner-left");


	(elet == null ? $(".dateget") : $(elet).find(".dateget")).removeClass("hasDatepicker");
	(elet == null ? $(".dateget") : $(elet).find(".dateget")).datepicker();
	/*
	{
beforeShow: function() {setTimeout( 
function () {$('.ui-datepicker').css('z-index', 9999);}, 100 );
	}}
	*/
	try {//ie8 bug
		(elet == null ? $("._button") : $(elet).find("._button")).each(function () {
			if (this.tagName.toUpperCase() != 'INPUT') $(this).attr("type", "button");
			if ($(this).hasClass('ui-button')) { return; };
			if ($(this).attr("tag") == null || $(this).attr("tag") == "") $(this).show().button();
			else $(this).show().button({
				icons: {
					primary: $(this).attr("tag")
				}
			});
		});
	} catch (ex) { }
	try {//ie8 bug
		(elet == null ? $("._sel") : $(elet).find("._sel")).each(function () {
			//alert($(this).next()[0].outerHTML);
			//if($(this).next().hasClass('ui-selectmenu-button'))$(this).next().remove();

			InitSel(this);//alert($(this).next()[0].outerHTML);
		});
	} catch (ex) { }
	if (elet != null) {
		if ($(elet).hasClass("ftdiv") || $(elet).hasClass("ftselect")) {
			$(elet).find("tr").hover(function () {
				if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", __d_win_hovercolor);
			}
				, function () {
					if ($(this).attr('onmouseout') != null && $(this).attr('onmouseout').indexOf('#ffffff') > 0) $(this).css("background-color", "#ffffff");
					else {
						if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", __d_win_bgcolor);
					}
				});
			$(elet).find("div").css({ "border-radius": "3px", "-moz-border-radius": "3px", "-webkit-border-radius": "3px" });
			$(elet).find("input,select,textarea").focus(function () {
				if (!$(this).hasClass('nohover')) $(this).css("background-color", "#F3FFC4").blur(function () {
					$(this).css("background-color", "#ffffff");
				});
			});
		}
		if ($(elet).hasClass("fttable")) {
			$(elet).find("tr").hover(function () {
				if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", "#ffffcc");
			}
				, function () {
					if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", "#ffffff");
				});
		}
	}
	else {
		$(".ftdiv tr,.ftselect tr").hover(function () {
			if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", __d_win_hovercolor);
		}
			, function () {
				if ($(this).attr('onmouseout') != null && $(this).attr('onmouseout').indexOf('#ffffff') > 0) $(this).css("background-color", "#ffffff");
		...
		$(".ftdiv div,.ftselect div").css({ "border-radius": "3px", "-moz-border-radius": "3px", "-webkit-border-radius": "3px" });
		$(".ftdiv input,.ftselect input,.ftdiv select,.ftselect select,.ftdiv textarea,.ftselect textarea").focus(function () {
			if (!$(this).hasClass('nohover')) $(this).css({ "background-color": "#F3FFC4", "border-color": "#ffbe7d" }).blur(function () {
				$(this).css({ "background-color": "#FFFFFF", "border-color": "#aaaaaa" });
			});
		});
		$(".fttable tr").hover(function () {
			if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", "#ffffcc");
		}
			, function () {
				if (!$(this).hasClass("nohover") && this.style.display != 'none') $(this).css("background-color", "#ffffff");
...
function StyleRow(obj) {
	if (!$(obj).hasClass("nohover")) {
		$(obj).hover(function () {
			$(this).css("background-color", __d_win_hovercolor);
		}
			, function () {
				$(this).css("background-color", $(obj).hasClass("pagerow") ? '#ffffff' : __d_win_bgcolor);
			});
	}
	$(obj).find(".dateget").removeClass("hasDatepicker");
	$(obj).find(".dateget").datepicker();
	$(obj).find("input,select,textarea").focus(function () {
		if (!$(this).hasClass('nohover')) $(this).css({ "background-color": "#F3FFC4", "border-color": "#ffbe7d" }).blur(function () {
			$(this).css({ "background-color": "#FFFFFF", "border-color": "#aaaaaa" });
		});
	});
...
function newDialog(id, type, title, ismodel) {
	...
	if (type == null) {
		...
		s = "<div id=\"" + id + "\" title=\"" + title + "\" style=\"padding-top:40px;text-align:center\"><img src=\"/_ft/_ftres/progress.gif\"/></div>";
	}
...
function showDialog(id, title, width, isaddform, idadd, ismodal, isresizeable, urlHeight) {
	var ele = null;
	var isUrl = false;
	if (urlHeight == null) urlHeight = 700;
	if (id.indexOf('/') >= 0) {
		var frameid = 'url' + Math.ceil(Math.random() * 100000);
		ele = document.createElement('iframe');
		if (id.indexOf('?') > 0) ele.src = id + '&' + frameid + '&' + new Date().getTime();
		else ele.src = id + '?' + frameid + '&' + new Date().getTime();
		ele.id = frameid;
		ele.style.height = urlHeight;
		isUrl = true;
	}
	else { ele = O(id); }
...
function _loading2suc(evaljs, val) {
	val = (val == null) ? "" : (":" + val);
	info("操作成功" + val, 4, $("#loading")[0], evaljs);
}
function _loading2fai(val, evaljs) {
...
function info(val, type, el, evaljs) {
	...
	$(ele).html("<img src='/_ft/_ftres/ui/" + (type == 1 ? "alert1.gif" : (type == 2 ? "alert0.gif" : (type == 3 ? "error.gif" : (type == 4 ? "success.gif" : "failed.png")))) + "'>&nbsp;" + val);
	$(ele).dialog({
		modal: true,
		resizable: false,
		show: 200,
...
function eleVal(eleid, eleval, rowrate) {
	...
	if (eleval == null) {
		...
		if (rowrate == null || rowrate == 0) {
			eleValBase(obj, objtag, objtype, eleval);
		}
		else {
...
function eleValBase(obj, objtag, objtype, val) {
	if (val == "FT{NORESULT}") {
...
function eleClear(eleid, expectids, forcehiddenids) {
	$("#" + eleid).find("*").each(function () {
		...
		if (objtag == "INPUT" && (objtype == "" || objtype == "TEXT" || objtype == "PASSWORD")) {
			...
			if (forcehiddenids != null && ("," + forcehiddenids + ",").indexOf("," + this.id + ",") >= 0) this.value = "";
		}
...
function listLoading(eleid) {
	$("#" + eleid).html("<div align=center style='padding:50px'><img src='/_ft/_ftres/progress.gif'/></div>");
}
