function dl_load(page, orderby, ordertype, isexport, isexportsave, esfilecap, _cq, ftpartid, DefaultOrderBy, List_Code, List_OPID, RerationTreeevalvals, sqlevalvals, IsTree, RerationTree, SiteID, sql, RowAll, _Conststr, MainTable, ExportMax, RateNumType, NumsPerPage, JSTableHead, HeadIsShow, SelectType, ColorSelect, ColorDefault, RateNumType, JSTableTail, TurnPage, FirstPage, PrePage, NextPage, LastPage, CountZero, TurnIsShow, BlockDataDefine, CacuRowData,LoadEndJS,CustomConnection,loadingimg,custurnbtm,custurntop) {
    ...
    if (isexport != null && isexport == true && isexportsave == 0) {
        ...
        if (listtempform == null) {
            var temformhtml = "";
            temformhtml += "<form id='listexporttempform' style='display:none' action='/_ftpub/ftformop' target='dhf" + ftpartid + "' method='post'>";
   ...
   function (data, textStatus) {
       ...
       if (data.indexOf("#fail:") == 0) {
           $("#data_" + ftpartid).html("<div align='center' style='padding:20px 0px 20px 0px'>" + decodeURIComponent(data.substring(6)) + "</div>");
           return false;
       }
       var json = eval("(" + data + ")");
       ...
       if (turnisshow == 1 && pagesize > 0) {
           if($.trim(custurntop)!="")eval("turntop=listturnpagetop_" + ftpartid + "(decodeURIComponent(json['turntop']))");
           if($.trim(custurnbtm)!="")eval("tableturn=listturnpage_" + ftpartid + "(decodeURIComponent(json['turnbtm']))");
           else{
       ...
       for (var i = 0; i < json["data"].length; i++) {
           ...
           if (BlockDataDefine != null && BlockDataDefine != '') {
               BlockHTML += decodeURIComponent(row).replace("{check}", eval("listtd0inner_" + ftpartid + "(fid)"));
               continue;
           }
           var rowdata = "";
           tablesavecolids += "," + fid;
           ...
           for (var j = 0; j < row.length; j++) {
               ...
               switch (j) {
                   case 0: eval("rowdata+=listtd1_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 1: eval("rowdata+=listtd2_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 2: eval("rowdata+=listtd3_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 3: eval("rowdata+=listtd4_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 4: eval("rowdata+=listtd5_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 5: eval("rowdata+=listtd6_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 6: eval("rowdata+=listtd7_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 7: eval("rowdata+=listtd8_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 8: eval("rowdata+=listtd9_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 9: eval("rowdata+=listtd10_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 10: eval("rowdata+=listtd11_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   case 11: eval("rowdata+=listtd12_" + ftpartid + "(col[1],decodeURIComponent(col[0]));"); break;
                   default: eval("rowdata+=listtdother_" + ftpartid + "(col[1],decodeURIComponent(col[0]));");
               }
           }
       ...
       var tablecacu = '';
       eval("tablecacu=listtrcacu_" + ftpartid + "(decodeURIComponent(json['cacu']))");
...
function dl_delete(obj, siteid, partid) {
    ...
    document.getElementById("dlf" + partid).action = "/_ftpub/ftformop";
...
function dl_saveto(obj, tabletag, siteid, partid) {
    ...
    newDialog("loading", "loading");
    document.getElementById("dli" + partid).action = "/_ftpub/ftformop";
...
function dl_export_savefiledel(partid, fileid) {
    newDialog('loading', 'loading');
    $.post("/_ftpub/ftajax", { "fileid": fileid, "ajaxtype": "List_ExportSaveFiles_Del" }, function (data, textStatus) {
...
function dl_export_saveuserbind(fileid) {
    ...
    $.post("/_ftpub/ftajax", { "ajaxtype": "List_ExportSaveFiles_Tree", "fileid": fileid }, function (data, textStatus) {
        closeDialog("info");
...
function dl_export_saveuserbind_do(fileid) {
    ...
    $.post("/_ftpub/ftajax", { "fileid": fileid, "userids": s, "ajaxtype": "List_ExportSaveFiles_Bind" }, function (data, textStatus) {
...
function dl_export_savestart(partid) {
    ...
    if ($("#" + id)[0] != null) $("#" + id).remove();
    var s = "<div id=\"" + id + "\" style=\"display:block;text-align:left;padding-top:30px\" class='ftdiv layui-from'>";
    s += "显示的文件名称：<input type=text class=_input layui-input style='width:250px;margin-right:4px'/>";
    s += '<button class="_button tip layui-btn" tag="ui-icon-clock" onclick="newDialog(\'loading\',\'loading\');load_' + partid + '(1,null,null,true,1,$(this).prev().val())">开始生成</button>';
    s += "</div>";
    $(s).appendTo($(document.body));
    StyleInit(O(id));
    $(O(id)).find("._button").each(function () {
...
function dl_export_savefiles(partid) {
    ...
    $.post("/_ftpub/ftajax", { "partid": partid, "ajaxtype": "ListExportSave" }, function (data, textStatus) {
...
function dl_export_all(partid) {
    ...
    if ($("#" + id)[0] != null) $("#" + id).remove();
    var s = "<div id=\"" + id + "\" style=\"text-align:right\" class=\"layui-from\">";
    s += "<input type=hidden autofocus >";
    s += '<button class="_button tip layui-btn" style="margin-right:10px" tag="ui-icon-extlink" title="数据量较少且网络条件较好时，可选择直接导出" onclick="load_' + partid + '(1,null,null,true)">直接导出</button>';
    s += '<button class="_button tip layui-btn" tag="ui-icon-disk" title="在服务器生成数据文件，生成后可随时下载，生成的数据文件且可与指定用户分享<br>适用于导出大数据文件，或者相同的数据需多次导出时使用" onclick="dl_export_savestart(\'' + partid + '\')">生成再下载</button>';
