...
namespace FTFrame.WorkFlow.Core
{
    public class WFC
    {
        public static void Push(Const.WorkFlowInstance WFI, string WorkID, string SubmitUser, string DepartInUser, string DepartID)
        {
            DB db = new DB();
            db.Open();
            ST st = null;
            ...
            {
                string sql = null;
                sql = "select count(*) from " + Const.Table.FC_WorkFlow_Step + " where FlowFID='" + str.D2DD(WFI.FlowFid) + "' and StepID=0";
                ...
                ArrayList JudgeUser = GetJudgeUser(db, WorkID, WFI.FlowFid, 0, DepartInUser, DepartID);
                st = db.GetTransaction();
                sql = "insert into " + Const.Table.FC_WorkFlow_Detail + "(WORKID,FLOWFID,FLOWNUM,STEPID,ALTERTIME)";
                sql += "values('" + str.D2DD(WorkID) + "','" + str.D2DD(WFI.FlowFid) + "'," + WFI.FlowNum + ",-1,'" + str.GetDateTimeMil() + "')";
                db.ExecSql(sql, st);
        ...
        public static string Judge(Const.WorkFlowInstance WFI, string WorkID, string UserID, Const.FlowResultType FlResultType, string FlMimo, string DepartInUser, string DepartID)
        {
            DB db = new DB();
            db.Open();
            ST st = null;
            try
            {
                ...
                if (FlResultType.Equals(Const.FlowResultType.PASS))//同意
                {
                    ...
                    {
                        ArrayList JudgeUser = GetJudgeUser(db, WorkID, WFI.FlowFid, StepID + 2, DepartInUser, DepartID);
                        st = db.GetTransaction();
                        sql = "delete from " + Const.Table.FC_WorkFlow_User_Dy + " where WORKID='" + str.D2DD(WorkID) + "'";
                        db.ExecSql(sql, st);
                        sql = "insert into " + Const.Table.FC_WorkFlow_Detail + "(WORKID,FLOWFID,FLOWNUM,STEPID,ALTERTIME,USERID,FLRESULT,FLMIMO)";
                        sql += "values('" + str.D2DD(WorkID) + "','" + str.D2DD(FlowFID) + "'," + WFI.FlowNum + "," + (StepID + 1) + ",'" + str.GetDateTimeMil() + "','" + str.D2DD(UserID) + "','同意','" + str.D2DD(FlMimo) + "')";
                        db.ExecSql(sql, st);
                ...
                else if (FlResultType.Equals(Const.FlowResultType.LASTSTEP))//退回给上级
                {
                    ...
                    int StepID = dr.GetInt32(0);
                    dr.Close();
                    if (StepID == -1)//上级就是本人，无法退还，不操作
                    {

                    }
                    else//上级审批的用户重新载入
                    {
                        ArrayList JudgeUser = GetJudgeUser(db, WorkID, FlowFID, StepID, DepartInUser, DepartID);
                        st = db.GetTransaction();
                        sql = "delete from " + Const.Table.FC_WorkFlow_User_Dy + " where WORKID='" + str.D2DD(WorkID) + "'";
                        db.ExecSql(sql, st);
                        sql = "insert into " + Const.Table.FC_WorkFlow_Detail + "(WORKID,FLOWFID,FLOWNUM,STEPID,ALTERTIME,USERID,FLRESULT,FLMIMO)";
                        sql += "values('" + str.D2DD(WorkID) + "','" + str.D2DD(FlowFID) + "'," + WFI.FlowNum + "," + (StepID - 1) + ",'" + str.GetDateTimeMil() + "','" + str.D2DD(UserID) + "','退回给上级','" + str.D2DD(FlMimo) + "')";
                        db.ExecSql(sql, st);
                        foreach (string NewUserID in JudgeUser)
                        {
                            sql = "insert into " + Const.Table.FC_WorkFlow_User_Dy + "(WORKID,USERID,ADDTIME,FLOWFID)";
                ...
                else if (FlResultType.Equals(Const.FlowResultType.REJECT))//退回给本人
                {
                    ArrayList JudgeUser = GetJudgeUser(db, WorkID, FlowFID, 0, DepartInUser, DepartID);
                    st = db.GetTransaction();
                    sql = "delete from " + Const.Table.FC_WorkFlow_User_Dy + " where WORKID='" + str.D2DD(WorkID) + "'";
                    db.ExecSql(sql, st);
                    sql = "insert into " + Const.Table.FC_WorkFlow_Detail + "(WORKID,FLOWFID,FLOWNUM,STEPID,ALTERTIME,USERID,FLRESULT,FLMIMO)";
                    sql += "values('" + str.D2DD(WorkID) + "','" + str.D2DD(FlowFID) + "'," + WFI.FlowNum + ",0,'" + str.GetDateTimeMil() + "','" + str.D2DD(UserID) + "','退回给本人','" + str.D2DD(FlMimo) + "')";
                    db.ExecSql(sql, st);
                    foreach (string NewUserID in JudgeUser)
                    {
                        sql = "insert into " + Const.Table.FC_WorkFlow_User_Dy + "(WORKID,USERID,ADDTIME,FLOWFID)";
        ...
        public static string Reset(Const.WorkFlowInstance WFI, string WorkID, string UserID, string DepartInUser, string DepartID)
        {
            DB db = new DB();
            db.Open();
            ST st = null;
            try
            {
                ...
                db.ExecSql(sql, st);
                sql = "insert into " + Const.Table.FC_WorkFlow_Detail + "(WORKID,FLOWFID,FLOWNUM,STEPID,ALTERTIME,USERID,FLRESULT,FLMIMO)";
                sql += "values('" + str.D2DD(WorkID) + "','" + str.D2DD(WFI.FlowFid) + "'," + WFI.FlowNum + ",-1,'" + str.GetDateTimeMil() + "','" + str.D2DD(UserID) + "','重置审批','')";
                db.ExecSql(sql, st);
                foreach (string NewUserID in JudgeUser)
                {
                    sql = "insert into " + Const.Table.FC_WorkFlow_User_Dy + "(WORKID,USERID,ADDTIME,FLOWFID)";
        ...
        public static string Cancel(Const.WorkFlowInstance WFI, string WorkID, string UserID)
        {
            DB db = new DB();
            db.Open();
            ST st = null;
            try
            {
                ...
                db.ExecSql(sql, st);
                sql = "insert into " + Const.Table.FC_WorkFlow_Detail + "(WORKID,FLOWFID,FLOWNUM,STEPID,ALTERTIME,USERID,FLRESULT,FLMIMO)";
                sql += "values('" + str.D2DD(WorkID) + "','" + str.D2DD(WFI.FlowFid) + "'," + WFI.FlowNum + ",-1,'" + str.GetDateTimeMil() + "','" + str.D2DD(UserID) + "','取消审批','')";
                db.ExecSql(sql, st);
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (!dr.IsDBNull("USERID"))//除去UserID为Null不显示，表示发起审批
                        al.Add(new string[] { Project.Core.User.UserShowName(dr.GetString("USERID")), dr.GetString("FLRESULT"), dr.GetString("FLMIMO"), str.GetDateTime(dr.GetValue("ALTERTIME")) });
                }
                dr.Close();
                ...
                while (dr.Read())
                {
                    ArrayList JudgeUser = GetJudgeUser(db2, WorkID, FlowFID, dr.GetInt32(0), DepartInUser, DepartID);
                    string UserCaps = "";
                    foreach (string UserID in JudgeUser)
                    {
                        UserCaps += " " + Project.Core.User.UserShowName(UserID);
                    }
                    JudgeUser.Clear();
                    JudgeUser = null;
                    if (dr.GetStringNoNULL(1).StartsWith("@code(")) al.Add(new string[] { UserCaps });
                    else
                    {
        ...
        private static string GetGroupCaption(DB db, string FlowFid, int StepID, string UserID, string DepartID)
        {
            ...
            {
                Caption += " " + Project.Core.User.GetDepartNameByDepartID(StepDepartID);
            }
            if (!Caption.Equals("")) Caption = Caption.Substring(1);
            return Caption;
        }
        private static ArrayList GetJudgeUser(DB db, string WorkId, string FlowFid, int StepID, string UserID, string DepartID)
        {
            string sql = null;
            sql = "select UserID from " + Const.Table.FC_WorkFlow_User + " where FlowFID='" + str.D2DD(FlowFid) + "' and StepID=" + StepID;
            ArrayList User_List = new ArrayList();
            ...
            dr.Close();
            if (StepRoleName != null && StepRoleName.StartsWith("@code("))
            {
                return (ArrayList)Code.Get(StepRoleName.Replace("$1", FlowFid).Replace("$2", WorkId).Replace("$3", StepID.ToString()),null);
            }
        ...
        private static ArrayList UserFromProjectManager(string ProjectID)
        {
            DB db = new DB();
            db.Open();
            try
            {
                ArrayList al = new ArrayList();
                string sql = "select USERID from PB_MANAGER where PROID='" + str.D2DD(ProjectID) + "'";
                DR dr = db.OpenRecord(sql);
                while (dr.Read())
                {
                    al.Add(dr.GetString(0));
        ...
        private static void WorkFlow_UpdateStat(string WorkID, DB db2, ST st, string TableName, string PrimaryColumn, int StatValue, Const.WorkFlowInstance WFI)
        {
            string sql = "update " + TableName + " set flow=" + StatValue + " where " + PrimaryColumn + "='" + str.D2DD(WorkID) + "'";
            db2.ExecSql(sql, st);
        }
        private static void WorkFlowJudge_Remind(Const.WorkFlowInstance WFI, string WorkID)
        {
            DB db = new DB();
            db.Open();
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    string UserCap = Project.Core.User.UserShowName(FromUserID);
        ...
        private static void WorkFlowEnd_Remind(Const.WorkFlowInstance WFI, string WorkID)
        {
            DB db = new DB();
            db.Open();
            try
        ...
        private static void WorkFlowReject_Remind(Const.WorkFlowInstance WFI, string WorkID)
        {
            DB db = new DB();
            db.Open();
            try
